...
$Behavior.globalThemeInit = function() {
	...
	$(document).click(function(e) {
	    $target = $(e.target);
	    var is = $target.is('.ym-share-link');
	    var isChild = $target.parents('.ym-share-link').length;
	
	    if(is || isChild) {
	    	//ignore
	    } else {
			$('.share_popupWrapper').hide(1,function() {  // first hide all `.hide`
			   //$('#'+ id +'.hide').show(); // then show the element with id `#1`
			});
			$('.share_popup_arrow.share_popup_bottom').hide(1,function() {  // first hide all `.hide`
			   //$('#'+ id +'.hide').show(); // then show the element with id `#1`
			});
	    }
	...
	// var pageHeight = screen.height;
	 var menuHeight = $('.ym-menu-slide').height();
	if($('.ym-share-link').length){
		$('.ym-share-link').click(function(){
			$('.share_popupWrapper').hide(1,function() {  // first hide all `.hide`
			   //$('#'+ id +'.hide').show(); // then show the element with id `#1`
			});
			$('.share_popup_arrow.share_popup_bottom').hide(1,function() {  // first hide all `.hide`
			...
			//add right css for class 'share_popupWrapper'
            // check the position of the content
            
            var pos = {x:$(this).offset().left, y:$(this).offset().top};
            var size = {x:$(this).width(), y:$(this).height()};
            var dir = {cx:0,cy:0};
            dir.cx = $(this).offset().left + $(this).width()/2;
            dir.cy = $(this).offset().top;
            var ele = $(this).find('.share_popupWrapper'); 
            
            var left = 0;
            if($(window).width() - dir.cx > 130){
            	var px = $(this).width()/2 - ele.width()/2;
            	left =  px + 'px';
            } else {
            	var px = $(this).width()/2 - ele.width()/2 - (ele.width()/2 - $(this).width()/2);
            	left =  px + 'px';
            }
            
            $(this).find('.share_popupWrapper').css('right', 'auto');
            $(this).find('.share_popupWrapper').css('left', left);
			
			$(this).find('.share_popupWrapper').toggle();
			$(this).find('.share_popup_arrow.share_popup_bottom').toggle();			
		});
	}
};