...
<?php /* Cached: March 15, 2014, 7:45 am */ ?>
<?php 
/**
 * [PHPFOX_HEADER]
 * 
...
<?php if (isset ( $this->_aVars['sActivityFeedHeader'] )): ?>
...
<?php if (isset ( $this->_aVars['bForceFormOnly'] ) && $this->_aVars['bForceFormOnly']): ?>
	<?php
						Phpfox::getLib('template')->getBuiltFile('wall.block.form');						
						?>
<?php else: ?>
<?php if (! isset ( $this->_aVars['bIsViewMore'] ) || ! $this->_aVars['bIsViewMore']): ?>
	<?php
						Phpfox::getLib('template')->getBuiltFile('wall.block.filter');						
						?>
<?php endif; ?>

<?php if (Phpfox ::getService('wall.feed')->timeline()): ?>
...
<?php if (( ! PHPFOX_IS_AJAX && Phpfox ::getService('wall.feed')->timeline()) || Phpfox ::getLib('request')->get('resettimeline')): ?>
...
<?php if (count((array)$this->_aVars['aFeedTimeline']['left'])):  $this->_aPhpfoxVars['iteration']['iFeed'] = 0;  foreach ((array) $this->_aVars['aFeedTimeline']['left'] as $this->_aVars['aFeed']):  $this->_aPhpfoxVars['iteration']['iFeed']++; ?>
					...
					<div class="timeline_feed_row">
						...
						<?php
						Phpfox::getLib('template')->getBuiltFile('wall.block.timeline');						
						?>
					</div>		
<?php endforeach; endif; ?>
				</div>
...
<?php endif; ?>
				...
				<div class="js_feed_view_more_entry_holder">
					<?php
						Phpfox::getLib('template')->getBuiltFile('wall.block.entry');						
						?>
...
<?php else: ?>
					...
					<a <?php if (! PHPFOX_IS_AJAX && isset ( $this->_aVars['bForceReloadOnPage'] ) && $this->_aVars['bForceReloadOnPage']): ?> style="text-indent:-1000px; overflow:hidden; background:transparent; border:0px;"<?php endif; ?> href="<?php if (Phpfox ::getLib('module')->getFullControllerName() == 'core.index-visitor'):  echo Phpfox::getLib('phpfox.url')->makeUrl('core.index-visitor', array('page' => $this->_aVars['iFeedNextPage']));  else:  echo Phpfox::getLib('phpfox.url')->makeUrl('current', array('page' => $this->_aVars['iFeedNextPage']));  endif; ?>" onclick="$(this).hide(); $('#feed_view_more_loader').show(); $.ajaxCall('wall.viewMore', 'page=<?php echo $this->_aVars['iFeedNextPage'];  if (defined ( 'PHPFOX_IS_USER_PROFILE' ) && isset ( $this->_aVars['aUser']['user_id'] )): ?>&profile_user_id=<?php echo $this->_aVars['aUser']['user_id'];  endif;  if (isset ( $this->_aVars['aFeedCallback']['module'] )): ?>&callback_module_id=<?php echo $this->_aVars['aFeedCallback']['module']; ?>&callback_item_id=<?php echo $this->_aVars['aFeedCallback']['item_id'];  endif; ?>&year=<?php echo $this->_aVars['sTimelineYear']; ?>&month=<?php echo $this->_aVars['sTimelineMonth']; ?>'+ '&viewId=' + wallFillterViewId, 'GET'); return false;" class="global_view_more no_ajax_link"><?php echo Phpfox::getPhrase('feed.view_more'); ?></a>
<?php endif; ?>
				</div>
<?php else: ?>
<?php if (Phpfox ::getService('wall.feed')->timeline()): ?>
...
<?php endif; ?>
<?php if (Phpfox ::getParam('feed.refresh_activity_feed') > 0 && Phpfox ::getLib('module')->getFullControllerName() == 'core.index-member'): ?>
<?php if (Phpfox ::VERSION >= '3.6.0'): ?>
			<script type="text/javascript">
				$Behavior.reloadActivityFeedAdvWall = function(){
					callReloadActivityFeed();
				}	
			</script>
<?php else: ?>
			<script type="text/javascript">
...
<?php if (Phpfox ::VERSION >= '3.6.0'): ?>
			<script type="text/javascript">
			 $Behavior.loadDisplayBlockAdvWall = function(){
<?php if (Phpfox ::isUser()): ?>
				Typeahead.data = <?php echo $this->_aVars['aJSONFriends']; ?>;
				Typeahead.emotions = <?php echo $this->_aVars['aEmoticons']; ?>;
<?php endif; ?>
				iProfileUserId  = '<?php if (defined ( 'PHPFOX_IS_USER_PROFILE' ) && isset ( $this->_aVars['aUser']['user_id'] )):  echo $this->_aVars['aUser']['user_id'];  endif; ?>';
...
<?php if (! Phpfox ::getService('wall.feed')->timeline() && ! PHPFOX_IS_AJAX && defined ( 'PHPFOX_IS_USER_PROFILE' )): ?>
	<script type="text/javascript">
	//$Behavior.doFilterAdvWall = function(){
		//callDoFilter('all');
	//}	
	</script>
<?php endif; ?>
