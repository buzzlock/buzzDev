...
<?php endif; ?>
					...
					<!-- Adv Wall -->
						...
						<div class="_uiCommentComposer">
							<div class="_uiMentionTypeAheadContainer">
								...
								<div class="_uiTypeaheaMentiondArea">
									<div class="innerWrap">
										<textarea cols="60" rows="1" name="val[plain_text]" class="js_comment_feed_textarea _uiMentionTypeahead text" id="js_feed_comment_form_textarea_<?php echo $this->_aVars['aFeed']['feed_id']; ?>"><?php if (isset ( $this->_aVars['sFeedType'] ) && $this->_aVars['sFeedType'] == 'view' && Phpfox ::getUserParam('comment.wysiwyg_on_comments') && Phpfox ::getParam('core.wysiwyg') == 'tiny_mce'):  else:  echo Phpfox::getPhrase('feed.write_a_comment');  endif; ?></textarea>
										<div style="position:absolute;" class="js_feed_comment_process_form"><?php echo Phpfox::getPhrase('feed.adding_your_comment');  echo Phpfox::getLib('phpfox.image.helper')->display(array('theme' => 'ajax/add.gif')); ?></div>
									</div>
								</div>
								<input type="hidden" name="val[text]" class="_uiMentionHidden"/>
							</div>
						</div>
					</div>
					<!-- Adv Wall -->
