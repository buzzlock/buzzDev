...
{if !PHPFOX_IS_AJAX && (!isset($isHideProfileHeader) || $isHideProfileHeader != 'yes') }
<ul class="ym-feed-header">
        <li>
            <a href="#" onclick="ynmtMobileTemplate.getStatusBlock(); return false;"><i class="icon-status"></i>{phrase var='mobiletemplate.status'}</a>
        </li>
        <li>
            <a href="#" onclick="ynmtMobileTemplate.clickPhoto(); return false;"><i class="icon-photos"></i>{phrase var='mobiletemplate.photos'}</a>
        </li>
        {if Phpfox::getParam('feed.enable_check_in') && Phpfox::getParam('core.google_api_key') != '' && Phpfox::getLib('module')->getFullControllerName() != 'pages.view' && Phpfox::getLib('module')->getFullControllerName() != 'event.view'}
        <li>
            <a href="#" onclick="ynmtMobileTemplate.initCheckIn(); return false;"><i class="icon-checkin"></i>{phrase var='mobiletemplate.checkin'}</a>
        </li>
        {/if}
    </ul>
    {/if}
...
{*
	...
	{/if} -->*}
	{if Phpfox::isUser() && !defined('PHPFOX_IS_USER_PROFILE') && !PHPFOX_IS_AJAX && !defined('PHPFOX_IS_PAGES_VIEW')}
		{plugin call='mobiletemplate.check_sort_feed'}
	{/if}
	...
	{else}	
		...
		{else}
			{foreach from=$aFeeds name=iFeed item=aFeed}
				...
				<div class="js_feed_view_more_entry_holder ym-feed-item">
