...
{if !phpfox::isMobile()}
        <div id="currentAlbum">
            <div id="currentAlbumHeader">
                <span id="dynamicalbumtitle" class="title">{$album_info.title}</span>
                &nbsp;
                <span class="by">{if isset($album_info.playlist_id)}{phrase var="musicsharing.created_by"}{else}{phrase var="musicsharing.uploaded_by"} {/if}</span>&nbsp;<a target="_blank" href="{url link=$album_info.user_name}" class="artist" id="dynamicartistname">{$album_info.full_name}</a>
            </div>
            <div id="currentAlbumImg" style="height: {$iPlheight}px; text-align: center; min-width: 204px;">
...
<div id="Comment" {if phpfox::isMobile()} class="comment-on-mobile" {/if}>
    ...
    {/if}
        ...
        <script type="text/javascript">
            $Behavior.MusicSharingDDTabContent = function(){l}
...
{/if}
<div class="clear"></div>
{plugin call='musicsharing.template_default_controller_listen_extra_info'}
