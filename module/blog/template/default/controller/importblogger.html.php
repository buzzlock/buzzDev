...
<div id="loading" style="display: none; margin-left: 200px; text-align: center; width: 320px; height: 320px; padding-top: 3 0px;">
 ...
 <script type="text/javascript">
      $Behavior.importBloggerBlog = function(){l}
...
<form onsubmit="sending_request();" enctype="multipart/form-data" class="global_form" action="{url link='current'}" method="post">
   <h1>{phrase var='blog.import_blogger'}</h1>
 <h3 style="border-bottom:0 solid #758AB7;">{phrase var='blog.have_a_blogger_export_file'} </h3>
   <div style="padding: 10px;">
      {phrase var='blog.choose_a_file_from_your_computer'}: ({phrase var='blog.maximum_size'})
        <input id="import_blog" type="file" name="import_blog"> <br/>
        <input type="submit" value="{phrase var='blog.upload_file_and_import'}" class="button" name="b_import" />
    </div>
</form>
  <div class="table_clear">
  	<ul class="table_clear_button">
   		<li>
   			<input style="margin-top:10px;" type="button" class="button" value="{phrase var='blog.back'}" onclick="window.location.href = '{url link='blog.import'}';">
   		</li>
   	</ul>
   	<div class="clear"></div>
 	</div>
