...
<div id="js_fundraising_entry{$aCampaign.campaign_id}"{if !isset($bFundraisingView)} class="js_fundraising_parent image_hover_holder {if ($phpfox.iteration.fundraising%3)==2}ynfr-item-mid{/if}{if $sTagType == 'fundraising_profile'} ynfr-profile{/if}{if $aCampaign.is_approved != 1} {/if}"{/if}>	
	<div class="ynfr-image-campaign">
		...
		<a href="{permalink module='fundraising' id=$aCampaign.campaign_id title=$aCampaign.title}" title="{$aCampaign.title|clean}">
			<span style="background:url({img return_url=true server_id=$aCampaign.server_id path='core.url_pic' file=$aCampaign.image_path suffix='_200'}) no-repeat top center;display:block;text-indent:-99999px;height:110px;width:165px;">
