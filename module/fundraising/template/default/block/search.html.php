...
$Core.remakePostUrl = function(){            
    ...
    $("#fundraising_search_form").attr('action', url);
}
</script>
<style type="text/css">
    .fundraising_date_picker{
        background: url({/literal}{$corepath}{literal}module/fundraising/static/image/calendar.gif) no-repeat top left;            
    }    
</style>
{/literal}
<div class="fundraising_date_picker"></div>
<form id="fundraising_search_form" method="post" onsubmit="$Core.remakePostUrl(); if($('#search_keywords').val()=='{phrase var='fundraising.search_fundraising_dot'}'){l}$('#search_keywords').val('');{r}">
    <input type="hidden" value="1" name="search[submit]">
    
    <table class="search_table" border="0" cellpadding="0" cellspacing="5">
        <tr>
            <td>{phrase var='fundraising.keywords'}:</td>            
        </tr>
        <tr>
            <td>
...
" onfocus="if($('#search_keywords').val()=='{phrase var='fundraising.search_fundraising_dot'}'){l}$('#search_keywords').val('');{r}" onblur="if($('#search_keywords').val()==''){l}$('#search_keywords').val('{phrase var='fundraising.search_fundraising_dot'}');{r}">
            </td>
        </tr>
               
        <tr>
            <td>{phrase var='fundraising.category'}:</td>
        </tr>
        <tr>
            <td>
                <select id="search_category" onchange="$Core.remakePostUrl();">
                <option {if $iCategoryFundraisingView==0}selected{/if} value="0">{phrase var='fundraising.all'}</option>
                {foreach from=$aCategories item=aCategory}
                    <option {if $iCategoryFundraisingView==$aCategory.category_id}selected{/if} value="{$aCategory.category_id}">{$aCategory.name}</option>
                {/foreach}
                </select>
            </td>
        </tr>
        {if $sView != 'pending'}
        <tr>
            <td>{phrase var='fundraising.status'}:</td>
        </tr>
        <tr>
            <td>
                <select id="search_status" onchange="$Core.remakePostUrl();">
                <option {if $iStatus==0}selected{/if} value="0">{phrase var='fundraising.all'}</option>
                <option {if $iStatus==1}selected{/if} value="1">{phrase var='fundraising.closed'}</option>
                <option {if $iStatus==2}selected{/if} value="2">{phrase var='fundraising.on_going'}</option>
                <option {if $iStatus==3}selected{/if} value="3">{phrase var='fundraising.victory'}</option>                
                </select>
            </td>
        </tr>
        {/if}
        <tr>
            <td>
                <input id="chk_search_by_date" type="checkbox" onclick="$('.search_by_date').toggle();"/> {phrase var='fundraising.search_by_date'}.                
            </td>
        </tr>
        <tr>        
        <tr class="search_by_date">
            <td>{phrase var='fundraising.from'}:</td>
        </tr>
        <tr class="search_by_date">
            <td>
        ...
        <tr class="search_by_date">
            <td>{phrase var='fundraising.to'}:</td>
        </tr>
        <tr class="search_by_date">
            <td>
                <div style="position: relative;">
    ...
    <div class="p_top_8">
        <input name="search[submit]" value="{phrase var='fundraising.submit'}" class="button" type="submit" />
    </div>    
</form>
{literal}
<script type="text/javascript">
