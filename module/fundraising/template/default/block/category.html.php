...
<div class="sub_section_menu">
	...
	{foreach from=$aCategories item=aCategory}
		<li class="{if $iCategoryFundraisingView == $aCategory.category_id} active{/if}"><a href="{$aCategory.url}" class="ajax_link">{$aCategory.title|convert|clean}</a>
                {if isset($aCategory.sub) && count($aCategory.sub)}
                <ul>
                      {foreach from=$aCategory.sub item=aSubCategory key=iKey}                    
                    <li class="{if $iCategoryFundraisingView == $aSubCategory.category_id} active{/if} {if isset($sModule)}{$sModule}_{/if}subcategory_{$aCategory.category_id} special_subcategory" {if $iKey >= Phpfox::getParam('fundraising.subcategories_to_show_at_first')}style="display:none;"{/if}>
                      <a href="{$aSubCategory.url}" id="{if isset($sModule)}{$sModule}_{/if}subcategory_{$aSubCategory.category_id}" class="ajax_link">
                                {$aSubCategory.title|convert|clean}
                      </a>
                    </li>
                      {/foreach}
          
                      {if $iKey >= Phpfox::getParam('fundraising.subcategories_to_show_at_first') && Phpfox::getParam('fundraising.subcategories_to_show_at_first') > 0}
