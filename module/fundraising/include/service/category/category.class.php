   ...
   public function __construct()
	...
	private function _get($iParentId, $iActive = null)
	{
		...
		if (count($aCategories))
		{
			...
			else 
			{
				...
				$this->_sOutput .= '<select name="val[category][' . $iParentId . ']" class="js_mp_category_list ynfr required" id="js_mp_id_' . $iParentId . '">' . "\n";
				$this->_sOutput .= '<option value="">' . ($iParentId === 0 ? Phpfox::getPhrase('fundraising.select') : Phpfox::getPhrase('fundraising.select_a_sub_category')) . ':</option>' . "\n";
			}
			...
			foreach ($aCategories as $iKey => $aCategory)
			{
				...
				if ($this->_sDisplay == 'option')
				{
					$this->_sOutput .= '<option value="' . $aCategory['category_id'] . '" id="js_mp_category_item_' . $aCategory['category_id'] . '">' . ($this->_iCnt > 0 ? str_repeat('&nbsp;', ($this->_iCnt * 2)) . ' ' : '') . Phpfox::getLib('locale')->convert($aCategory['title']) . '</option>' . "\n";
					//$this->_sOutput .= $this->_get($aCategory['category_id'], $iActive);					
				}
				elseif ($this->_sDisplay == 'admincp')
				{
					$sOutput .= '<li><img src="' . Phpfox::getLib('template')->getStyle('image', 'misc/draggable.png') . '" alt="" /> <input type="hidden" name="order[' . $aCategory['category_id'] . ']" value="' . $aCategory['ordering'] . '" class="js_mp_order" /><a href="#?id=' . $aCategory['category_id'] . '" class="js_drop_down">' . Phpfox::getLib('locale')->convert($aCategory['title']) . '</a>' . $this->_get($aCategory['category_id'], $iActive) . '</li>' . "\n";
				}
				else 
				{				
					$this->_sOutput .= '<option value="' . $aCategory['category_id'] . '" id="js_mp_category_item_' . $aCategory['category_id'] . '">' . Phpfox::getLib('locale')->convert($aCategory['title']) . '</option>' . "\n";
				}
			}
	...
	public function getCategoriesByCampaignId($iCampaignId) {
		...
		{			
			foreach ($aCategories as $aCategory)
			{				
				$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategory['title']), Phpfox::permalink('fundraising.category', $aCategory['category_id'], $aCategory['title']));
			}
		}		
		else 
		{			
			$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategories[0]['title']), Phpfox::permalink('fundraising.category', $aCategories[0]['category_id'], $aCategories[0]['title']));
		}
	...
	public function getForBrowse($iCategoryId = null)
	{
		...
		{					
			$aCategories = $this->database()->select('mc.category_id, mc.title')
			...
			foreach ($aCategories as $iKey => $aCategory)
			{
				...
				//if ($sCategory === null)
				{
					$aCategories[$iKey]['sub'] = $this->database()->select('mc.category_id, mc.title')
						->from($this->_sTable, 'mc')
