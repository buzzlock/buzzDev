<?php
	Phpfox::getLib('setting')->setParam('photo.show_info_on_mouseover',false);
?>
				...
				{if Phpfox::getParam('photo.auto_crop_photo') && !Phpfox::getParam('photo.show_info_on_mouseover')}
					<div class="photo_clip_holder_border{if Phpfox::getParam('photo.show_info_on_mouseover')}_big{/if}">
						<a href="{$aPhoto.link}{if isset($iForceAlbumId)}albumid_{$iForceAlbumId}/{/if}"
						   ...
						   style="background-image:url({img return_url=true defer=true server_id=$aPhoto.server_id path='photo.url_photo' file=$aPhoto.destination suffix='_240'})">
						    
						    {*img defer=true server_id=$aPhoto.server_id path='photo.url_photo' file=$aPhoto.destination suffix='_240' title=$aPhoto.title*}
