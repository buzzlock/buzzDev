	...
	{		
		...
		if ($aVals = $this->request()->getArray('val'))
		{
			if ($bIsEdit)
			{
				...
				{
					$this->url()->send('admincp.contest.category.add', array('id' => $aCategory['category_id']), Phpfox::getPhrase('contest.category_successfully_updated'));
				}
			}
			else 
			{
				...
				{
					$this->url()->send('admincp.contest.category.add', null, Phpfox::getPhrase('contest.category_successfully_added'));
				}
			}
		}
		
		$this->template()->setTitle(($bIsEdit ? Phpfox::getPhrase('contest.edit_a_category') : Phpfox::getPhrase('contest.create_a_new_category')))
			->setBreadcrumb(($bIsEdit ? Phpfox::getPhrase('contest.edit_a_category') : Phpfox::getPhrase('contest.create_a_new_category')), $this->url()->makeUrl('admincp.ontest'))
			->assign(array(
