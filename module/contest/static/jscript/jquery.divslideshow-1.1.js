(function($){	
	...
	{		
		...
		$(slideShow).find('.dssControl').click(function(){	
			...
			$(slideShow).find('.dssSlideContainer').stop();
			...
			if(dir) //left or right arrow
			{
				...
				var gotoPage = 0;
				if(currentPage + dir < max && currentPage + dir >= 0)
				{
					gotoPage = currentPage + dir;
				}
				else if (currentPage + dir >= max)
				{
					gotoPage = (currentPage + dir) % max;
				}
				else if(currentPage + dir < 0)
				{
					gotoPage = max - 1;
				}
			}				
			...
			//requeue autoslide
			// for(i = gotoPage + 1; i < Math.floor(numSlide*options.loop); i++)
			// {
			// 	$(slideShow).find('.dssSlideContainer').delay(3000);				
			// 	$.divSlideShow.slideTo(slideShow, i % numSlide, options);
			// }
		});		
	};
