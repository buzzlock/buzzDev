...
<div class="js_video_parent main_videochannel_div_container {if phpfox::isMobile()}yv-mobile-feature{/if} {if isset($aVideo.is_sponsor) && $aVideo.is_sponsor}row_sponsored_image{/if} {if isset($aVideo.is_featured) && $aVideo.is_featured}row_featured_image{/if}" id="js_video_id_{$aVideo.video_id}">	
...
$(this).parents('.js_video_parent:first').find('.row_pending_link:first').hide(); return false;">{phrase var='videochannel.approve'}</a>
						...
						|| (defined('PHPFOX_IS_PAGES_VIEW') && Phpfox::getService('pages')->isAdmin('' . $aPage.page_id . ''))
							...
							<li class="item_delete">
								<a href="#" title="{phrase var='videochannel.delete_this_video'}" onclick="if (confirm('{phrase var='videochannel.are_you_sure' phpfox_squote=true}')) $.ajaxCall('videochannel.delete', 'video_id={$aVideo.video_id}'); return false;">{phrase var='videochannel.delete'}</a>
							</li>
						{/if}						
			...
			<a href="{$aVideo.link}" class="row_sub_link js_video_title_{$aVideo.video_id}" id="js_video_title_{$aVideo.video_id}" title="{$aVideo.title}" >{$aVideo.title|clean|shorten:30:'...'|split:20}</a>
			<div class="extra_info_link">
				...
				<?php
					...
					if (strlen($aVideo['full_name']) > 20):
						$aVideo['full_name'] = substr($aVideo['full_name'], 0, 20) . "...";
