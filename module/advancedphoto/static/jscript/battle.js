...
$Core.photoBattle = 
{
	...
	{		
		...
		if (!isset(this.aPhotos[iId]))
		{
			...
			$.ajaxCall('advancedphoto.battle', sParams + '&category=' + this.mCategory);			
	...
	getImage: function(iId, iCnt)
	{
		...
		sHtml += '</a>';
		sHtml += '<div class="extra_info">' + oTranslations['advancedphoto.added_on_time_stamp_by_full_name'].replace('{time_stamp}', this.aPhotos[iId][iCnt]['time_stamp']).replace('{full_name}', this.aPhotos[iId][iCnt]['full_name']) + '</div>';
