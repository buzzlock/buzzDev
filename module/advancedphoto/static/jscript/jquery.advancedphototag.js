...
$Core.photo_tag =
{
	...
	{		
		...
		$(function() 
		{
			...
			this.sHtml += '<form id="NoteAddForm" method="post" action="#" onsubmit="$(\'#noteform\').hide(); $(\'' + $Core.photo_tag.aParams['id'] + '\').imgAreaSelect({ hide: true }); $(this).ajaxCall(\'advancedphoto.addPhotoTag\'); return false;">';
			...
			{
				this.sHtml += '<div class="extra_info"><a href="#" onclick="$(\'#js_tag_user_id\').val(\'' + $Core.photo_tag.aParams['user_id'] + '\'); $(\'#noteform\').hide(); $(\'' + $Core.photo_tag.aParams['id'] + '\').imgAreaSelect({ hide: true }); $(\'#NoteAddForm\').ajaxCall(\'advancedphoto.addPhotoTag\'); return false;">' + oTranslations['photo.click_here_to_tag_as_yourself'] + '</a></div>';
			}			
			this.sHtml += '</form>';
			this.sHtml += '</div>';
