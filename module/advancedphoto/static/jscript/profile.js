...
$Behavior.photoUserBrowser = function()
{
	...
	{			
		...
		{			
			...
			$.ajaxCall('advancedphoto.browseUserAlbum', $Core.parseUrlString(window.location.hash));
		}
		else
		{
			$('#js_user_photos').html($.ajaxProcess(oTranslations['language.loading'], 'large'));
			
			$.ajaxCall('advancedphoto.browseUserPhotos', $Core.parseUrlString(window.location.hash));			
		}
	}	
	
	$('#js_photo_view_more_albums').click(function()
	{		
		$.ajaxCall('advancedphoto.browseUserAlbum', $Core.getRequests(this.href));
