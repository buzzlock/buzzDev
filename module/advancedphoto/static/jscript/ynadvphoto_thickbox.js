...
$Behavior.ynadvphotoAddDraggableToBoxes = function()
{
    $('a.ynadvphoto_thickbox').click(function()
    {
    	...
    	var bReturn = ynadvphoto_tb_show('', sHref, this);
...
function ynadvphoto_tb_show(caption, url, thisObject, sForceMessage, bForceNoCilck) 
{	
	var baseURL;
	if (url.indexOf("?")!==-1)
	{
	...
	if ($(thisObject).hasClass('photo_holder_image') && !empty($(thisObject).attr('rel')))
	{
		...
		var queryString = '' + getParam('sGlobalTokenName') + '[call]=advancedphoto.view&width=940&req2=' + $(thisObject).attr('rel') + '&theater=true&no_remove_box=true' + (sUserId != null && isset(sUserId[1]) ? '&userid='+sUserId[1] :'') + (sAlbumId != null && isset(sAlbumId[1]) ? '&albumid='+sAlbumId[1] :'');
