...
$Core.PhotoRate = 
{
	...
	getImage: function(iCnt)
	{
		...
		sHtml += '<div class="extra_info">' + oTranslations['advancedphoto.added_on_time_stamp_by_full_name'].replace('{time_stamp}', this.oPhotos[iCnt]['time_stamp']).replace('{full_name}', this.oPhotos[iCnt]['full_name']) + '</div>';
		...
		else
		{
			...
			sStat += '<div class="extra_info t_center">' + oTranslations['advancedphoto.with_a_total_of_total_vote_votes'].replace('{total_vote}', this.oPhotos[iCnt]['total_vote']) + '</div>';
		}
	...
	processRatings: function()
	{
		...
		$.ajaxCall('advancedphoto.getPhotosForRating', sParams + $Core.parseUrlString(window.location.hash));
	},
