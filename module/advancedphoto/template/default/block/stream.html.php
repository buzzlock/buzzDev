...
<div class="photo_stream_holder">
	...
	{if count($aStreams.previous)}
		...
		{if $aPreviousPhoto.user_id == Phpfox::getUserId() ||
			...
			(Phpfox::getUserId() && Phpfox::getUserParam('advancedphoto.photo_mature_age_limit') <= Phpfox::getUserBy('age'))}
		<a href="{$aPreviousPhoto.link}" title="{phrase var='advancedphoto.title_by_full_name' title=$aPreviousPhoto.title full_name=$aPreviousPhoto.full_name|clean}">
		...
		<a href="{$aPreviousPhoto.link}"
		   onclick="tb_show('{phrase('advancedphoto.warning')}', $.ajaxBox('advancedphoto.warning', 'height=300&amp;width=350&amp;link={$aPreviousPhoto.link}'));
		    return false;">
		   {img theme='misc/no_access.png' width='75px'}
		</a>
		{elseif $aPreviousPhoto.mature == 2 && Phpfox::getUserParam('advancedphoto.photo_mature_age_limit') > Phpfox::getUserBy('age')}
		<a href="{$aPreviousPhoto.link}">
		    {img theme='misc/no_access.png' width='75px'}
		</a>
		{/if}
	...
	{foreach from=$aStreams.next item=aNextPhoto}
		...
		{if $aNextPhoto.user_id == Phpfox::getUserId() ||
			$aNextPhoto.mature == 0 ||
			(Phpfox::getUserId() && Phpfox::getUserParam('advancedphoto.photo_mature_age_limit') <= Phpfox::getUserBy('age'))}
		<a href="{$aNextPhoto.link}" title="{phrase var='advancedphoto.title_by_full_name' title=$aNextPhoto.title full_name=$aNextPhoto.full_name|clean}">
		...
		<a href="{$aNextPhoto.link}" 
		   onclick="tb_show('{phrase('advancedphoto.warning')}', $.ajaxBox('advancedphoto.warning', 'height=300&amp;width=350&amp;link={$aNextPhoto.link}'));
		    return false;">
		   {img theme='misc/no_access.png' width='75px'}
		</a>
		{elseif $aNextPhoto.mature == 2 && Phpfox::getUserParam('advancedphoto.photo_mature_age_limit') > Phpfox::getUserBy('age')}
		<a href="{$aNextPhoto.link}">
		    {img theme='misc/no_access.png' width='75px'}
		</a>
		{/if}
