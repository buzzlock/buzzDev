	...
	{literal}
		{
			...
			{			
				$('#js_photo_action').append('<option value="view_album" id="js_photo_view_this_album">' + oTranslations['advancedphoto.view_this_album'] + '</option>');
			}
			$('#js_create_new_album').ajaxCall('advancedphoto.addAlbum');	
		}
...
<div class="main_break">
	...
	<div class="t_right p_top_10" style="font-size:12pt; font-weight:bold;">
		- <a href="{$sSkipLink}">{phrase var='advancedphoto.skip_this_step'}</a>
	</div>	
	<form method="post" action="{url link='advancedphoto.upload.process'}">
		...
		{/if}	
		{plugin call='advancedphoto.template_controller_upload_form_process_hidden'}
		...
		{if count($aImages) > 1}				
		<h3>{phrase var='advancedphoto.global_photo_settings'}</h3>
		<p>{phrase var='advancedphoto.note_that_global_photo_settings_will_override_any_settings_saved_individually_below'}</p>
		
		{template file="advancedphoto.block.form" bGlobalSetting=true}
		
		<div class="table_clear">
			<input type="submit" value="{phrase var='advancedphoto.save_global_settings'}" class="button" name="save_global" />
		</div>	
		
		<br />
		
		<h3>{phrase var='advancedphoto.uploaded_photos'}</h3>
		{/if}	
		...
		<div class="{if is_int($phpfox.iteration.images/2)}row1{else}row2{/if}{if $phpfox.iteration.images == 1} row_first{/if}">			
			{template file='advancedphoto.block.form'}
		</div>	
		{/foreach}
		
		<div class="table_clear">
			<input type="submit" value="{phrase var='advancedphoto.save'}" class="button" name="save" /> {phrase var='advancedphoto.and_then'}
			<select name="val[action]" style="vertical-align:middle;">
			{if $iTotalNextBatch}
				<option value="process">{phrase var='advancedphoto.process_next_batch_total_left' total=$iTotalNextBatch}</option>
			{/if}
			{if isset($aImages.0.album_id) && $aImages.0.album_id > 0}
				<option value="view_album">{phrase var='advancedphoto.view_this_album'}</option>
			{/if}
				<option value="view_photo">{phrase var='advancedphoto.view_your_photos'}</option>			
				<option value="upload">{phrase var='advancedphoto.upload_new_images'}</option>
				{plugin call='advancedphoto.template_controller_upload_form_actions'}
			</select>
		</div>
	</form>
...
<div id="js_album_form" style="display:none;">
	...
	<form method="post" action="{url link='current'}" id="js_create_new_album" onsubmit="return addNewAlbum();">
		...
		<div class="table_clear">
			<input type="submit" value="{phrase var='advancedphoto.submit'}" class="button" /> - <a href="#" onclick="$('#js_album_form').hide(); $('#js_upload_form').show(); $('#content h1').html('{phrase var='advancedphoto.upload_photos' phpfox_squote=true}'); return false;">{phrase var='advancedphoto.cancel_lowercase'}</a>
		</div>
...
<div id="js_upload_form">
	...
	<div id="js_upload_inner_form">
		<form method="post" action="{url link='advancedphoto.frame'}" id="js_form" enctype="multipart/form-data" target="js_upload_frame" onsubmit="return startProcess(true, true);">
			...
			{if $aCallback === null && Phpfox::getUserParam('advancedphoto.can_create_photo_album')}
			<div class="table" id="album_table">
				<div class="table_left">
					{phrase var='advancedphoto.album'}:
				</div>
				<div class="table_right_text">
					...
					{phrase var='advancedphoto.or'} 
					</span>
					<a href="#" onclick="$('#js_upload_form').hide(); $('#js_album_form').show(); $('#content h1').html('{phrase var='advancedphoto.create_a_new_album' phpfox_squote=true}'); return false;">{phrase var='advancedphoto.create_a_new_one'}</a>.				
				</div>
			</div>		
			{/if}		
			<div class="table">
				...
				<div class="table_right">
					...
					<span id="swfUploadText">{phrase var='advancedphoto.select_photo_s'}</span> 
			...
			<div class="table_clear">				
				<div id="js_upload_form_outer">					
					...
					{/if}
					{phrase var='advancedphoto.and_then'}
					<select name="val[action]" id="js_photo_action" style="vertical-align:middle;">
						<option value="process">{phrase var='advancedphoto.process_your_photos'}</option>
						<option value="upload">{phrase var='advancedphoto.upload_more_photos'}</option>
						{if $iAlbumId}<option value="view_album" id="js_photo_view_this_album">{phrase var='advancedphoto.view_this_album'}</option>{/if}
						<option value="view_photo">{phrase var='advancedphoto.view_your_photos'}</option>
						{plugin call='advancedphoto.template_controller_upload_form_actions'}
					</select>
				</div>
			...
			{plugin call='advancedphoto.template_controller_upload_form_extra'}
		</form>
	</div>		
	<div id="js_uploaded_images" style="display:none;">
		<h3>{phrase var='advancedphoto.recently_uploaded'}</h3>
		<div class="label_flow" style="height:200px;">
			<div id="js_uploaded_images_data"></div>
		</div>
		<form method="post" action="{url link='advancedphoto.upload.process'}" id="js_post_form">
		{if $aCallback !== null}
			<div><input type="hidden" name="item" value="{$aCallback.group_id}" /></div>
			<div><input type="hidden" name="module" value="group" /></div>		
		{/if}
			...
			<div style="padding-top:5px; text-align:right;">
				<input type="submit" value="{phrase var='advancedphoto.process_your_photo_s'}" class="button" />
			</div>
		</form>	
	</div>		
</div>	
