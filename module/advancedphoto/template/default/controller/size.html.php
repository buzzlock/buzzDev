	...
	<div class="photo_view_all_sizes">
		...
		{foreach from=$aSizes item=aSize}
			<li class="{if $iDefaultSize == $aSize.actual} is_active{/if}">
				<a href="#" onclick="$('.photo_view_all_sizes li').removeClass('is_active'); $(this).parent().addClass('is_active'); $.ajaxCall('advancedphoto.viewAllSizes', 'id={$aPhoto.photo_id}&amp;size={$aSize.actual}&amp;replace=1', 'GET'); return false;">{$aSize.width} x {$aSize.height}</a>
			</li>		
		{/foreach}	
		</ul>
		<div class="clear"></div>
...
{/if}
		...
		<a href="{permalink module='photo' id=$aPhoto.photo_id title=$aPhoto.title}download/size_{$iDefaultSize}/" title="{phrase var='advancedphoto.download_this_image'}">
		{/if}
