 ...
 */
class Advancedphoto_Component_Controller_Frame extends Phpfox_Component
{
	/**
	...
	{	
		...
		if (($iFlood = Phpfox::getUserParam('advancedphoto.flood_control_photos')) !== 0)
		{
			...
			if (Phpfox::getLib('spam')->check($aFlood))
			{
				...
				Phpfox_Error::set(Phpfox::getPhrase('advancedphoto.uploading_photos_a_little_too_soon') . ' ' . Phpfox::getLib('spam')->getWaitTime());	
			}
		...
		$bIsInline = false;
		if (isset($aVals['action']) && $aVals['action'] == 'upload_advancedphoto_via_share')
		{
			$bIsInline = true;
		}		
				...
				{					
					...
					if ($iId = $oServicePhotoProcess->add(Phpfox::getUserId(), array_merge($aVals, $aImage)))
					{
						...
						$sFileName = $oFile->upload('image[' . $iKey . ']', 
							...
							(Phpfox::getParam('advancedphoto.rename_uploaded_photo_names') ? Phpfox::getUserBy('user_name') . '-' . $aPhoto['title'] : $iId),
		...
		if (count($aImages))
		{
			...
			if (isset($aVals['album_id']) && !empty($aVals['album_id']))
			{
				...
				if (!defined('PHPFOX_SKIP_FEED_ENTRY') && !Phpfox::getUserParam('advancedphoto.photo_must_be_approved'))
				{
