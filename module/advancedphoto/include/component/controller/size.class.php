 ...
 */
class Advancedphoto_Component_Controller_Size extends Phpfox_Component
{
	/**
	...
	public function process()
	{
		...
		Phpfox::getUserParam('advancedphoto.can_view_all_photo_sizes', true);
		
		$aCallback = $this->getParam('aCallback', null);	
		
		$aPhoto = Phpfox::getService('advancedphoto')->getPhoto((PHPFOX_IS_AJAX ? $this->request()->getInt('id') : $this->request()->get('req2')));
		...
		foreach (Phpfox::getParam('advancedphoto.photo_pic_sizes') as $iPhotoSize)
		{
		...
		$this->template()
			...
			->setBreadcrumb(Phpfox::getPhrase('advancedphoto.photo'), ($aCallback === null ? $this->url()->makeUrl('advancedphoto') : $this->url()->makeUrl($aCallback['url_home_photo'])))
	...
	{
		(($sPlugin = Phpfox_Plugin::get('advancedphoto.component_controller_size_clean')) ? eval($sPlugin) : false);
	}
}
