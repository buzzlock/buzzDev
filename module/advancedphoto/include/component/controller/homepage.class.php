 ...
 */
class Advancedphoto_Component_Controller_Homepage extends Phpfox_Component
{
	/**
	...
	{		
		...
		if (($iUnFeature = $this->request()->getInt('unfeature')) && Phpfox::getUserParam('advancedphoto.can_feature_photo'))
		{
			...
			{
				$this->url()->send('photo', null, Phpfox::getPhrase('advancedphoto.photo_successfully_unfeatured'));
			}
		}
		...
		$bIsTagSearch = false;
		$sPhotoUrl = ($bIsUserProfile ? $this->url()->makeUrl($aUser['user_name'], 'photo') : ($aParentModule === null ? $this->url()->makeUrl('advancedphoto') : $aParentModule['url'] . 'photo/'));
...
//					'jquery.wookmark.js' => 'module_advancedphoto'
	...
	{
		(($sPlugin = Phpfox_Plugin::get('advancedphoto.component_controller_homepage_clean')) ? eval($sPlugin) : false);
	}
}
