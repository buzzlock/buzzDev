 ...
 */
class Advancedphoto_Component_Ajax_Ajax extends Phpfox_Ajax
{
    public function viewAllSizes()
	{
		if (!$this->get('replace'))
		{
			$this->setTitle(Phpfox::getPhrase('advancedphoto.view_all_sizes'));
		}
		Phpfox::getComponent('advancedphoto.size', array(), 'controller');
		if ($this->get('replace'))
		{
    ...
    {
		$this->setTitle(Phpfox::getPhrase('advancedphoto.create_a_new_photo_album'));	
    	// Only users can view this form.
		Phpfox::isUser(true);
		// Only users with this specific user group perm. can view this form.
		Phpfox::getUserParam('advancedphoto.can_create_photo_album', true);
		// Display the block form
		Phpfox::getBlock('advancedphoto.album');
		
		$this->call('<script type="text/javascript"> $Behavior.swfUploadLoader = function(){}; $(\'*\').unbind(); $.each($Behavior, function() {		this(this); });	</script>');
...
//		$this->call('<script type="text/javascript">$Core.loadInit();</script>');
    ...
    public function addAlbum()
    {
		...
		if (!$bAllowedAlbums)
		{
			...
			$this->alert(Phpfox::getPhrase('advancedphoto.you_have_reached_your_limit_you_are_currently_unable_to_create_new_photo_albums'));
		...
		{
			// $this->alert(Phpfox::getPhrase('advancedphoto.album_successfully_created'));
    ...
    {
		Phpfox::getLib('module')->getComponent('advancedphoto.index', $this->getAll(), 'controller');
    ...
    {
		Phpfox::getLib('module')->getComponent('advancedphoto.profile', $this->getAll(), 'controller');
    ...
    {
		Phpfox::getLib('module')->getComponent('advancedphoto.album', $this->getAll(), 'controller');
    ...
    {
		Phpfox::getBlock('advancedphoto.featured');
	
		$this->html('#js_block_content_featured_photo', $this->getContent(false))->call('setTimeout("$.ajaxCall(\'advancedphoto.refreshFeaturedImage\');", ' + Phpfox::getService('advancedphoto')->getFeaturedRefreshTime() + ');');
    }
    ...
    public function updateAlbumTitle()
    {
		...
		{
		    $this->alert(Phpfox::getPhrase('advancedphoto.add_a_title'));
	
		    return false;
		}
	
		if (Phpfox::getService('user.auth')->hasAccess('photo_album', 'album_id', $this->get('album_id'), 'advancedphoto.can_edit_own_photo_album', 'advancedphoto.can_edit_other_photo_albums') && Phpfox::getService('advancedphoto.album.process')->updateTitle($this->get('album_id'), $this->get('quick_edit_input')))
		{
    ...
    public function updateAlbum()
    {
		...
		if (Phpfox::getService('user.auth')->hasAccess('photo_album', 'album_id', $aVals['album_id'], 'advancedphoto.can_edit_own_photo_album', 'advancedphoto.can_edit_other_photo_albums') && Phpfox::getService('advancedphoto.album.process')->update($aVals['album_id'], $aVals))
		{
			...
			$this->hide('#js_album_edit_form')
				...
				->html('#js_updating_album', ' - <a href="#" onclick="$(\'#js_album_edit_form\').hide(); $(\'#js_user_photo_albums\').show(); return false;">' . Phpfox::getPhrase('advancedphoto.cancel') . '</a>');
		    }
		    else
		    {
			$this->html('#js_ge_edit_inner_title' . $aVals['album_id'], $oParseOutput->clean($oParseInput->clean($aVals['name'])))
				...
				->html('#js_updating_album', ' - <a href="#" id="js_album_cancel_edit">' . Phpfox::getPhrase('advancedphoto.cancel') . '</a>');
		    }
		}
    }
    ...
    public function updatePhoto()
    {
		...
		if(isset($aPostVals['ynadvphoto_hour']))
		{
			$aVals['ynadvphoto_month'] = $aPostVals['ynadvphoto_month'];
			$aVals['ynadvphoto_day'] = $aPostVals['ynadvphoto_day'];
			$aVals['ynadvphoto_year'] = $aPostVals['ynadvphoto_year'];
			$aVals['ynadvphoto_hour'] = $aPostVals['ynadvphoto_hour'];
			$aVals['ynadvphoto_minute'] = $aPostVals['ynadvphoto_minute'];
		}
		...
		if (($iUserId = Phpfox::getService('user.auth')->hasAccess('photo', 'photo_id', $aVals['photo_id'], 'advancedphoto.can_edit_own_photo', 'advancedphoto.can_edit_other_photo')) && Phpfox::getService('advancedphoto.process')->update($iUserId, $aVals['photo_id'], $aVals))
		{
		    $oParseInput = Phpfox::getLib('parse.input');
		    $oParseOutput = Phpfox::getLib('parse.output');
		    
		    $aPhoto = Phpfox::getService('advancedphoto')->getForEdit($aVals['photo_id']);
		    ...
		    {
		    	$this->call('window.location.href = "' . Phpfox::getLib('url')->permalink('advancedphoto', $aPhoto['photo_id'], Phpfox::getLib('parse.input')->clean($aVals['title'])) . '";');
		    }
		}
    }
    ...
    {
		if (Phpfox::getService('user.auth')->hasAccess('photo_album', 'album_id', $this->get('album_id'), 'advancedphoto.can_edit_own_photo_album', 'advancedphoto.can_edit_other_photo_albums') && Phpfox::getService('advancedphoto.album.process')->setCover($this->get('album_id'), $this->get('photo_id')))
		{
    ...
    public function deleteNewPhoto()
    {
		...
		// Delete the photo.
		if (Phpfox::getService('advancedphoto.process')->delete($this->get('id')))
		{
    ...
    public function deletePhoto()
    {
		...
		if (Phpfox::getService('advancedphoto.process')->delete($this->get('photo_id')))
		{
    ...
    public function editPhoto()
    {
		...
		if (Phpfox::getService('user.auth')->hasAccess('photo', 'photo_id', $this->get('photo_id'), 'advancedphoto.can_edit_own_photo', 'advancedphoto.can_edit_other_photo'))
		{
	    	Phpfox::getBlock('advancedphoto.edit-photo', array('ajax_photo_id' => $this->get('photo_id')));
	    	$this->setTitle(Phpfox::getPhrase('advancedphoto.editing_photo'));
	    	$this->call('<script type="text/javascript">$Core.loadInit();</script>');
		}
    }
    ...
    {		
    	Phpfox::getBlock('advancedphoto.warning');
    }
    ...
    public function getPhotosForRating()
    {
		...
		(($sPlugin = Phpfox_Plugin::get('advancedphoto.component_ajax_getphotosforrating_start')) ? eval($sPlugin) : false);
	
		$aPhotos = $this->get('photo_id');
	
		if (is_array($aPhotos) && Phpfox::getService('advancedphoto.rate.process')->add($this->get('photo_id')))
		{
		...
		$sPhotos = Phpfox::getService('advancedphoto.rate')->getJavaScript($sCategory);
	
		if ($sPhotos === false)
		{
		    $this->html('#site_content', '<div class="extra_info">' . Phpfox::getPhrase('advancedphoto.you_have_rated_all_the_available_images', array('phpfox_squote' => true)) . '</a>');
		}
		else
		{
		...
		(($sPlugin = Phpfox_Plugin::get('advancedphoto.component_ajax_getphotosforrating_end')) ? eval($sPlugin) : false);
    }
    ...
    public function battle()
    {
		...
		if (is_array($aPhotos) && Phpfox::getService('advancedphoto.battle.process')->add($aPhotos))
		{
		...
		$sJavaScript = Phpfox::getService('advancedphoto.battle')->getJavaScript($sCategory);
    ...
    public function getCategoryForEdit()
    {
		...
		$aCategory = Phpfox::getService('advancedphoto.category')->getCategory($this->get('id'));
	
		$this->call('$(\'#js_photo_category_' . $aCategory['parent_id'] . '\').attr(\'selected\', true);');
	
		$this->html('#js_photo_table_header', Phpfox::getPhrase('advancedphoto.editing_category') . ': ' . $aCategory['name'])
			->html('#js_photo_hidden', '<input type="hidden" name="val[edit_id]" value="' . $aCategory['category_id'] . '" />')
			->html('#js_photo_extra_button', ' <input type="button" name="" value="' . Phpfox::getPhrase('advancedphoto.cancel') . '" class="button" onclick="$(\'#js_photo_category_' . $aCategory['parent_id'] . '\').attr(\'selected\', false); $(\'#js_category_holder\').show(); $(\'#js_photo_table_header\').html(\'' . Phpfox::getPhrase('advancedphoto.add_a_photo_category') . '\'); $(\'#js_photo_extra_button\').html(\'\'); $(\'#js_photo_hidden\').html(\'\'); $(\'#name\').val(\'\');" /> <input type="submit" value="' . Phpfox::getPhrase('advancedphoto.delete') . '" onclick="return confirm(\'' . Phpfox::getPhrase('advancedphoto.are_you_sure') . '\');" class="button" name="val[delete]" />')
    ...
    public function approve()
    {
		...
		if (Phpfox::getService('advancedphoto.process')->approve($this->get('id')))
		{
			$this->alert(Phpfox::getPhrase('advancedphoto.photo_has_been_approved'), Phpfox::getPhrase('advancedphoto.photo_approved'), 300, 100, true);
    ...
    {
		Phpfox::getBlock('advancedphoto.new');
	
		$this->html('#' . $this->get('id'), $this->getContent(false));
		$this->call('$(\'#' . $this->get('id') . '\').parents(\'.block:first\').find(\'.bottom li a\').attr(\'href\', \'' . Phpfox::getLib('url')->makeUrl('advancedphoto') . '\');');
    }

    public function feature()
    {
		Phpfox::isUser(true);
		Phpfox::getUserParam('advancedphoto.can_feature_photo', true);
	
		if (Phpfox::getService('advancedphoto.process')->feature($this->get('photo_id'), $this->get('type')))
		{
		    if ($this->get('type') == '1')
		    {
				$sHtml = '<a href="#" title="' . Phpfox::getPhrase('advancedphoto.un_feature_this_photo') . '" onclick="$.ajaxCall(\'advancedphoto.feature\', \'photo_id=' . $this->get('photo_id') . '&amp;type=0\'); return false;">' . Phpfox::getPhrase('advancedphoto.un_feature') . '</a>';
		    }
		    else
		    {
				$sHtml = '<a href="#" title="' . Phpfox::getPhrase('advancedphoto.feature_this_photo') . '" onclick="$.ajaxCall(\'advancedphoto.feature\', \'photo_id=' . $this->get('photo_id') . '&amp;type=1\'); return false;">' . Phpfox::getPhrase('advancedphoto.feature') . '</a>';
		    }
	
		    $this->html('#js_photo_feature_' . $this->get('photo_id'), $sHtml)->alert(($this->get('type') == '1' ? Phpfox::getPhrase('advancedphoto.photo_successfully_featured') : Phpfox::getPhrase('advancedphoto.photo_successfully_un_featured')));
		    if ($this->get('type') == '1')
		    {
    ...
    {
		Phpfox::getUserParam('advancedphoto.can_sponsor_photo', true);
		// 0 = remove sponsor; 1 = add sponsor
		if (Phpfox::getService('advancedphoto.process')->sponsor($this->get('photo_id'), $this->get('type')))
		{
		    if ($this->get('type') == '1')
		    {
				Phpfox::getService('ad.process')->addSponsor(array('module' => 'photo', 'item_id' => $this->get('photo_id')));
				// image was sponsored
				$sHtml = '<a href="#" title="' . Phpfox::getPhrase('advancedphoto.unsponsor_this_photo') . '" onclick="$.ajaxCall(\'advancedphoto.sponsor\', \'photo_id=' . $this->get('photo_id') . '&amp;type=0\'); return false;">' . Phpfox::getPhrase('advancedphoto.unsponsor_this_photo') . '</a>';	
		    }
		    else
		    {
				Phpfox::getService('ad.process')->deleteAdminSponsor('photo', $this->get('photo_id'));
				$sHtml = '<a href="#" title="' . Phpfox::getPhrase('advancedphoto.unsponsor_this_photo') . '" onclick="$.ajaxCall(\'advancedphoto.sponsor\', \'photo_id=' . $this->get('photo_id') . '&amp;type=1\'); return false;">' . Phpfox::getPhrase('advancedphoto.sponsor_this_photo') . '</a>';
		    }
		    $this->html('#js_photo_sponsor_' . $this->get('photo_id'), $sHtml)->alert($this->get('type') == '1' ? Phpfox::getPhrase('advancedphoto.photo_successfully_sponsored') : Phpfox::getPhrase('advancedphoto.photo_successfully_un_sponsored'));
		    if($this->get('type') == '1')
		    {
    ...
    {
		Phpfox::isUser(true);
		if ($aPhoto = Phpfox::getService('advancedphoto.process')->rotate($this->get('photo_id'), $this->get('photo_cmd')))
		{
		    Phpfox::getService('advancedphoto.tag.process')->deleteAll($this->get('photo_id'));
	
		    $this->call('window.location.href = \'' . Phpfox::getLib('url')->permalink('advancedphoto', $aPhoto['photo_id'], $aPhoto['title']) . 'refresh_1/\';');
		}
    }
    ...
    public function addPhotoTag()
    {
		...
		if (($sReturn = Phpfox::getService('advancedphoto.tag.process')->add($aVals['tag'])))
		{
		    ...
		    $this->call('$Core.photo_tag.init({' . Phpfox::getService('advancedphoto.tag')->getJs($aVals['tag']['item_id']) . '});');
		}
    }

    public function removePhotoTag()
    {
		if ($iPhoto = Phpfox::getService('advancedphoto.tag.process')->delete($this->get('tag_id')))
		{
		    $this->call('$(\'.note\').remove(); $Core.photo_tag.init({' . Phpfox::getService('advancedphoto.tag')->getJs($iPhoto) . '});');
		}
    }
    ...
    public function process()
    {
		    ...
		    if ($aImage['completed'] == 'false')
		    {
				...
				if (isset($aPhoto['photo_id']))
				{
				    ...
				    foreach(Phpfox::getParam('advancedphoto.photo_pic_sizes') as $iSize)
				    {
						...
						if ($oImage->createThumbnail(Phpfox::getParam('photo.dir_photo') . sprintf($sFileName, ''), Phpfox::getParam('photo.dir_photo') . sprintf($sFileName, '_' . $iSize), $iSize, $iSize, true, ((Phpfox::getParam('advancedphoto.enabled_watermark_on_photos') && Phpfox::getParam('core.watermark_option') != 'none') ? (Phpfox::getParam('core.watermark_option') == 'image' ? 'force_skip' : true) : false)) === false)
						{
		...
		if ($iNotCompleted === 0)
		{
			...
			{
				(Phpfox::isModule('feed') ? $iFeedId = Phpfox::getService('feed.process')->callback($aCallback)->add('advancedphoto', $aPhoto['photo_id'], $aPhoto['privacy'], $aPhoto['privacy_comment'], (int) $this->get('parent_user_id', 0)) : null);			
		    ...
		    else if ($this->get('action') == 'upload_advancedphoto_via_share')
		    {
		    	// $aCallback = ($this->get('callback_module') ? Phpfox::callback($this->get('callback_module') . '.addPhoto', $this->get('callback_item_id')) : null);		    	
		    ...
		    {				
			    ...
			    {
					Phpfox::addMessage((count($aImages) == 1 ? Phpfox::getPhrase('advancedphoto.photo_successfully_uploaded') : Phpfox::getPhrase('advancedphoto.photos_successfully_uploaded')));
		
					$this->call('window.parent.location.href = \'' . Phpfox::getLib('url')->permalink('advancedphoto', $aPhoto['photo_id'], $aPhoto['title']) . 'userid_' . Phpfox::getUserId() . '/\';');
			    }
			    elseif ($this->get('action') == 'view_album' && isset($aImages[0]['album']))
			    {
					Phpfox::addMessage((count($aImages) == 1 ? Phpfox::getPhrase('advancedphoto.photo_successfully_uploaded') : Phpfox::getPhrase('advancedphoto.photos_successfully_uploaded')));
			
					$this->call('window.location.href = \'' . Phpfox::getLib('url')->permalink('advancedphoto.album', $aImages[0]['album']['album_id'], $aImages[0]['album']['name']) . '\';');
			    }
			    else
			    {
					Phpfox::addMessage((count($aImages) == 1 ? Phpfox::getPhrase('advancedphoto.photo_successfully_uploaded') : Phpfox::getPhrase('advancedphoto.photos_successfully_uploaded')));
					...
					if (Phpfox::getParam('advancedphoto.photo_upload_process'))
					{
						...
						$this->call('window.location.href = \'' . Phpfox::getLib('url')->makeUrl('advancedphoto', array('view' => 'my', 'mode' => 'edit', 'photos' => urlencode(base64_encode($sImages)))) . '\';');
					}
					else
					{		
						$this->call('window.location.href = \'' . Phpfox::getLib('url')->permalink('advancedphoto', $aPhoto['photo_id'], $aPhoto['title']) . 'userid_' . Phpfox::getUserId() . '/\';');
					}
			    }
		...
		{
		    $this->call('$(\'#js_progress_cache_holder\').html(\'\' + $.ajaxProcess(\'' . Phpfox::getPhrase('advancedphoto.processing_image_current_total', array('phpfox_squote' => true, 'current' => (count($aImages) - $iNotCompleted), 'total' => count($aImages))) . '\', \'large\') + \'\');');
		    ...
		    $this->call('$.ajaxCall(\'advancedphoto.process\', \'&action=' . $this->get('action') . '&js_disable_ajax_restart=true&photos=' . urlencode(base64_encode(json_encode($aImages))) . $sExtra . '\');');
		}
    }

    public function editAlbum()
    {
		$aAlbum = Phpfox::getService('advancedphoto.album')->getForEdit($this->get('id'));
		...
		if (isset($aAlbum['album_id']))
		{
		    ...
		    $this->template()->getTemplate('advancedphoto.block.form-album');
    ...
    {
    	Phpfox::getComponent('advancedphoto.view', array(), 'controller');
	...
	public function moderation()
	{
		...
		switch ($this->get('action'))
		{
			case 'approve':
				...
				{
					Phpfox::getService('advancedphoto.process')->approve($iId);
					$this->call('$(\'#js_photo_id_' . $iId . '\').remove();');					
				}								
				$sMessage = Phpfox::getPhrase('advancedphoto.photo_s_successfully_approved');
				break;			
			case 'delete':
				Phpfox::getUserParam('advancedphoto.can_delete_other_photos', true);
				foreach ((array) $this->get('item_moderate') as $iId)
				{
					Phpfox::getService('advancedphoto.process')->delete($iId);
					$this->call('$(\'#js_photo_id_' . $iId . '\').remove();');		
				}				
				$sMessage = Phpfox::getPhrase('advancedphoto.photo_s_successfully_deleted');
				break;
		}
	...
	public function massUpdate()
	{
		...
		foreach ($aVals as $iPhotoId => $aVal)
		{
			...
			if (isset($aPhoto['photo_id']))
			{
				...
				{
					Phpfox::getService('advancedphoto.process')->update($aPhoto['user_id'], $aPhoto['photo_id'], $aVal);	
				}
			}
		}
		...
		{
			$this->alert(Phpfox::getPhrase('advancedphoto.successfully_updated_photo_s'), Phpfox::getPhrase('advancedphoto.notice'), 300, 150, true);
	...
	public function getForAttachment()
	{
		...
		Phpfox::getBlock('advancedphoto.attachment');
	...
	public function attachToItem()
	{
		...
		$aPhoto = Phpfox::getService('advancedphoto')->getPhoto($this->get('photo-id'));
		
		if (!isset($aPhoto['photo_id']))
		{
			$this->alert(Phpfox::getPhrase('advancedphoto.unable_to_find_the_photo_you_are_looking_for'));
		...
		if ($aPhoto['user_id'] != Phpfox::getUserId())
		{
			$this->alert(Phpfox::getPhrase('advancedphoto.unable_to_import_this_photo'));
	...
	public function makeProfilePicture()
	{
		...
		if (Phpfox::getService('advancedphoto.process')->makeProfilePicture($this->get('photo_id')))
		{
			
		}
		
		Phpfox::addMessage(Phpfox::getPhrase('advancedphoto.profile_photo_successfully_updated'));
		$this->call('window.location.href = \'' . Phpfox::getLib('url')->makeUrl('user.photo') . '\';');
	}
    
    /**
     * Reload Random Photos block on index
     */
    public function reloadRandom()
    {
        $iLimit = Phpfox::getParam('advancedphoto.number_of_photos_displayed_on_blocks_newrandom_on_homepage');
        
        list($iCnt, $aRandomPhotos) = Phpfox::getService('advancedphoto')->getRandomPhotos($iLimit);
        
        foreach($aRandomPhotos as $aPhoto)
        {
            Phpfox::getLib('template')
    			->assign(array('aPhoto' => $aPhoto))
    			->getTemplate('advancedphoto.block.small-photo');
        }
        
        $sContent = $this->getContent(false);
        $sContent .= '<div class="clear"></div>
	<p class="advancedphoto-viewmore">
		<a href="#" onclick="$.ajaxCall(\'advancedphoto.reloadRandom\'); return false;"> '.Phpfox::getPhrase('advancedphoto.refresh').' </a>
	</p>';

        $this->html('#yn_random', $sContent);
        $this->call('$Core.loadInit();');
        $this->call('$("#yn_show_random").trigger("click");');
    }
}
