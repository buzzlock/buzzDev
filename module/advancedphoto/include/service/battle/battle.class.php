 ...
 */
class Advancedphoto_Service_Battle_Battle extends Phpfox_Service 
{
	/**
	 * Class constructor
	 */	
	...
	public function get($sCategory = null)
	{
		...
		if ($sCategory !== null)
		{
			$sCategoryIds = Phpfox::getService('advancedphoto.category')->getAllCategories($sCategory);
		...
		foreach ($aRows as $iKey => $aRow)
		{
			if ($iKey === 0)
			{
				$aRow['link'] = Phpfox::getLib('url')->makeUrl('advancedphoto.battle', array('w' => $aRow['photo_id'], 'l' => $aRows[1]['photo_id'], 'mode' => $sMode));
			}
			else 
			{
				$aRow['link'] = Phpfox::getLib('url')->makeUrl('advancedphoto.battle', array('w' => $aRow['photo_id'], 'l' => $aRows[0]['photo_id'], 'mode' => $sMode));
			}
	...
	private function _build($aPhoto)
	{
		...
		foreach ($aPhoto as $sKey => $sValue)
		{
			...
			{
				$sValue = Phpfox::getTime(Phpfox::getParam('advancedphoto.photo_image_details_time_stamp'), $aPhoto['time_stamp']);
			}
	...
	public function __call($sMethod, $aArguments)
	{
		 ...
		 */
		if ($sPlugin = Phpfox_Plugin::get('advancedphoto.service_battle_battle__call'))
		{
			return eval($sPlugin);
		}
