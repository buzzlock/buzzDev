 ...
 * @version 		$Id: advancedphoto.class.php 4532 2012-07-19 10:03:18Z Miguel_Espinoza $
 */
class Advancedphoto_Service_Photo extends Phpfox_Service 
{
	...
	public function getPhoto($sId, $iUserId = 0, $aCallback = null)
	{
		...
		*/
		...
		{
			$this->database()->select('lik.like_id AS is_liked, ')->leftJoin(Phpfox::getT('like'), 'lik', '(lik.type_id = \'photo\' OR lik.type_id = \'advancedphoto\') AND lik.item_id = p.photo_id AND lik.user_id = ' . Phpfox::getUserId());
		}	
	...
	{		
		...
		{
			$sQuery = ' AND advancedphoto.module_id = \'' . $this->database()->escape($aCallback['module_id']) . '\' AND advancedphoto.group_id = ' . (int) $aCallback['item_id'];
		}
		else
		{
			$sQuery = ' AND advancedphoto.group_id = 0 ';
		}
		
		if ($iAlbumId > 0)
		{
			$sQuery = ' AND advancedphoto.album_id = ' . (int) $iAlbumId;	
		}
		if ($iUserId > 0)
		{
			$sQuery .= ' AND advancedphoto.user_id = ' . (int)$iUserId;
		}
		...
		if (!Phpfox::isAdmin())
		{
			...
			*/
			...
			$sQuery .= '(advancedphoto.privacy = 0)';
			if (Phpfox::getParam('core.section_privacy_item_browsing'))
			{	
				$sQuery .= ' OR ';
				// 3 - "Only me" privacy
				$sQuery .= ' (advancedphoto.privacy = 3 AND advancedphoto.user_id = ' . Phpfox::getUserId() . ') '; 				
				...
				if ($iCnt > 0)
				{
					...
					$sQuery .= ' OR (advancedphoto.privacy = 1 AND advancedphoto.user_id IN ' . $sFriendsIn .')';
					
					// 2 - Friends of Frends
					$aFriendsOfFriends = Phpfox::getService('friend')->getFriendsOfFriends($sFriendsIn);
					$sIn = implode(',', $aFriendsOfFriends);
					$sQuery .= ' OR (advancedphoto.privacy = 2 AND advancedphoto.user_id IN (' . $sIn . '))';
					
					$aInList = Phpfox::getService('friend.list')->getUsersInAnyList();					
					if (!empty($aInList))
					{
						$sIn = implode(',', $aInList);
						$sQuery .= ' OR (advancedphoto.privacy = 4 AND advancedphoto.user_id IN (' . $sIn . '))';
					}
			...
			{
				$sQuery .= ' AND advancedphoto.privacy = 0';
			}
			$sQuery .= ')';			
		}		
		
		list($iPreviousCnt, $aPrevious) = $this->_getPhoto('AND advancedphoto.photo_id > ' . (int) $iPhotoId . $sQuery, 'ASC', (empty($sQuery) ? false : true), $iCategory);
		list($iNextCount, $aNext) = $this->_getPhoto('AND advancedphoto.photo_id < ' . (int) $iPhotoId . $sQuery, 'DESC', (empty($sQuery) ? false : true), $iCategory);				
	...
	{		
		if ($bNoPrivacy === true)
		{
			...
			{
				$this->database()->join(Phpfox::getT('photo_category_data'), 'pcd', 'pcd.photo_id = advancedphoto.photo_id AND pcd.category_id = ' . ((int)$iCategoryChecked));
			}
			...
			if ($iCategoryChecked !== null)
			{				
				$this->database()->select('pcd.category_id,')->join(Phpfox::getT('photo_category_data'), 'pcd', 'pcd.photo_id = advancedphoto.photo_id AND pcd.category_id = ' . (int)$iCategoryChecked);
			}
			$aPrevious = (array) $this->database()->select('advancedphoto.*')
				->from(Phpfox::getT('photo'), 'photo')
				->where(array($sCondition))
				->order('advancedphoto.photo_id ' . $sOrder)
				->execute('getSlaveRow');
		...
		$this->search()->setCondition('AND advancedphoto.view_id = 0 AND advancedphoto.group_id = 0 AND advancedphoto.type_id < 2 AND advancedphoto.privacy IN(%PRIVACY%)');
