 ...
 */
class Advancedphoto_Service_Album_Album extends Phpfox_Service 
{
	/**
	 * Class constructor
	 */	
	...
	public function getOtherAlbumsOfSameUploader($iUserId, $iLimit = 5)
	{
		$sCond = 'pa.privacy = 0  && pa.user_id = ' . $iUserId;
		$iIndex = rand(0,2);
		$aOrder = array(
			'pa.total_photo DESC',
			'pa.time_stamp DESC',
		...
		$sOrder = $aOrder[$iIndex];
		$iPage = 1;
		$iPageSize = $iLimit;
		return $this->get($sCond, $sOrder, $iPage, $iPageSize);
	}
	...
	public function getFeatured($iLimit = 4, $bNoStaticCache = false)
	{
		...
		if ($aFeatured !== null || !$bNoStaticCache)
		{
		...
		$sKey = 'featured_albums_' . $iLimit;
		$sType = 'featured_albums';
		if (!($aRows = $oCustomCache->get($sKey, $sType)))
		{
			$aRows = $this->database()->select('pa.*, p.destination, p.server_id, p.mature, ' . Phpfox::getUserField())
		...
		if (is_array($aRows) && count($aRows))
		{
			...
			{
				$aRow['link'] = Phpfox::permalink('advancedphoto.album', $aRow['album_id'], $aRow['name']);
				if ($iKey === $iLimit)
				{
					break;
				}
	...
	{		
		...
		(($sPlugin = Phpfox_Plugin::get('advancedphoto.service_album_album_get_count')) ? eval($sPlugin) : false);
		...
		if ($iCnt)
		{
			(($sPlugin = Phpfox_Plugin::get('advancedphoto.service_album_album_get_query')) ? eval($sPlugin) : false);
	...
	{
		(($sPlugin = Phpfox_Plugin::get('advancedphoto.service_album_album_getall')) ? eval($sPlugin) : false);
	...
	{
		(($sPlugin = Phpfox_Plugin::get('advancedphoto.service_album_album_getalbumcount')) ? eval($sPlugin) : false);
	...
	{
		(($sPlugin = Phpfox_Plugin::get('advancedphoto.service_album_album_getalbum')) ? eval($sPlugin) : false);
		
		$aAlbum = $this->database()->select('pa.*, pai.*')
			->from($this->_sTable, 'pa')
		...
		{
			$aTags = Phpfox::getService('tag')->getTagsById('album', $aAlbum['album_id']);	
			if (isset($aTags[$aAlbum['album_id']]))
			{
				$aAlbum['tag_list'] = $aTags[$aAlbum['album_id']];
			}
		}
	...
	public function getForView($iId, $bIsProfile = false)
	{
		...
		{
			$this->database()->select('l.like_id AS is_liked, ')->leftJoin(Phpfox::getT('like'), 'l', '(l.type_id = \'photo_album\' OR l.type_id = \'advancedphoto_album\') AND l.item_id = pa.album_id AND l.user_id = ' . Phpfox::getUserId());
		}		
		...
		if (!isset($aAlbum['album_id']) || $bForceCreation === true)
		{
			...
			if (!isset($aAlbum['album_id']))
			{
				$iId = $this->database()->insert(Phpfox::getT('photo_album'), array(
						...
						'name' => Phpfox::getPhrase('advancedphoto.profile_pictures'),//'Profile Pictures',
	...
	{
		(($sPlugin = Phpfox_Plugin::get('advancedphoto.service_album_album_getforedit')) ? eval($sPlugin) : false);
		...
		$aAlbum['ynadvphoto_time'] = Phpfox::getLib('date')->convertFromGmt($aAlbum['time_stamp'], 0);
			
		$aAlbum['ynadvphoto_month'] = date('n', $aAlbum['ynadvphoto_time']);
		$aAlbum['ynadvphoto_day'] = date('j', $aAlbum['ynadvphoto_time']);
		$aAlbum['ynadvphoto_year'] = date('Y', $aAlbum['ynadvphoto_time']);
		$aAlbum['ynadvphoto_hour'] = date('H', $aAlbum['ynadvphoto_time']);
		$aAlbum['ynadvphoto_minute'] = date('i', $aAlbum['ynadvphoto_time']);
	...
	public function inThisAlbum($iAlbumId, $iLimit = 7)
	{
	...
	public function __call($sMethod, $aArguments)
	{
		 ...
		 */
		if ($sPlugin = Phpfox_Plugin::get('advancedphoto.service_album__call'))
		{
			return eval($sPlugin);
		}
