 ...
 */
class Advancedphoto_Service_Category_Category extends Phpfox_Service 
{
	...
	public function getCategoriesById($iId, &$aCategories = null)
	{
		...
		{			
			foreach ($aCategories as $aCategory)
			{				
				$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategory['name']), Phpfox::permalink('advancedphoto.category', $aCategory['category_id'], $aCategory['name']), 'category_id' => $aCategory['category_id']);
			}
		}		
		else 
		{			
			$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategories[0]['name']), Phpfox::permalink('advancedphoto.category', $aCategories[0]['category_id'], $aCategories[0]['name']), 'category_id' => $aCategories[0]['category_id']);
		}
	...
	public function getForBrowse($iCategoryId = null, $sIsRatingArea = null)
	{
		...
		{					
			...
			foreach ($aCategories as $iKey => $aCategory)
			{
				...
				{
					$aCategories[$iKey]['url'] = Phpfox::permalink('advancedphoto.category', $aCategory['category_id'], $aCategory['name']);
				}
				else 
				{
					$aCategories[$iKey]['url'] = Phpfox::permalink('advancedphoto.' . $sIsRatingArea . '.category', $aCategory['category_id'], $aCategory['name']);
				}
				...
				//if ($sCategory === null)
				{
					...
					foreach ($aCategories[$iKey]['sub'] as $iSubKey => $aSubCategory)
					{
						if ($sIsRatingArea === null)
						{
							$aCategories[$iKey]['sub'][$iSubKey]['url'] = Phpfox::permalink('advancedphoto.category', $aSubCategory['category_id'], $aSubCategory['name']);
						}
						else 
						{
							$aCategories[$iKey]['sub'][$iSubKey]['url'] = Phpfox::permalink('advancedphoto.' . $sIsRatingArea . '.category', $aSubCategory['category_id'], $aSubCategory['name']);
						}
					}
				}
			}
	...
	{		
		...
		foreach ($aCategories as $aCategory)
		{
			...
			else 
			{
				if ($bAnchor === true)
				{
					$sCategories .= '<li><a href="' . Phpfox::getLib('url')->makeUrl('advancedphoto', $mUrl) . '" class="js_photo_category" id="js_photo_category_' . $aCategory['category_id'] . '">' . Phpfox::getLib('locale')->convert($aCategory['name']) . '</a>' . $this->_get($aCategory['category_id'], $bAnchor) . '</li>';
				}
				else 
				{
	...
	public function __call($sMethod, $aArguments)
	{
		 ...
		 */
		if ($sPlugin = Phpfox_Plugin::get('advancedphoto.service_category__call'))
		{
			return eval($sPlugin);
		}
