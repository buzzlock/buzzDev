 ...
 */
class Advancedphoto_Service_Api extends Phpfox_Service 
{
	/**
	 * Class constructor
	 */	
	...
	{
		// Check permission
		if ($this->_oApi->isAllowed('advancedphoto.add_photo') == false)
		{
			return $this->_oApi->error('advancedphoto.add_photo', 'User did not allow to upload photos on their behalf.');
		}	
		...
		$aImage = $oFile->load('photo', array(
			'jpg',
			'gif',
			'png'), (Phpfox::getUserParam('advancedphoto.photo_max_upload_size') === 0 ? null : (Phpfox::getUserParam('advancedphoto.photo_max_upload_size') / 1024))
		);
		...
		if (!empty($aErrors))
		{
			return $this->_oApi->error('advancedphoto.photo_add_photo_load', array_pop($aErrors));
		}

		$aReturnPhotos = array();
		if ($iId = Phpfox::getService('advancedphoto.process')->add(PHPFOX_APP_USER_ID, $aImage))
		{
			...
			if (!empty($aErrors))
			{
				return $this->_oApi->error('advancedphoto.photo_add_photo_upload', array_pop($aErrors));
			}
			...
			Phpfox::getService('advancedphoto.process')->update(Phpfox::getUserId(), $iId, array(
		...
		return $this->_oApi->error('advancedphoto.add_photo_process', 'Could not add photo to process');
	...
	public function __call($sMethod, $aArguments)
	{
		 ...
		 */
		if ($sPlugin = Phpfox_Plugin::get('advancedphoto.service_api__call'))
		{
			eval($sPlugin);
			return;
		}
