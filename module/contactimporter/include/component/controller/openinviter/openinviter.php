...
class openinviter
{
    ...
    private function checkLoginCredentials($user)
    {
        ...
        if ($this->currentPlugin['requirement'])
        {
            ...
            {
                $this->internalError = Phpfox::getPhrase('contactimporter.please_enter_the_full_email_not_just_the_username');
                return false;
            }
            elseif ($this->currentPlugin['requirement'] == 'user' AND $is_email)
            {
                $this->internalError = Phpfox::getPhrase('contactimporter.please_enter_just_the_username_not_the_full_email');
                return false;
            }
        }
        if ($this->currentPlugin['allowed_domains'] AND $is_email)
        {
            ...
            if (!$temp)
            {
                $this->internalError = Phpfox::getPhrase('contactimporter.strong_user_domain_strong_is_not_a_valid_domain_for_this_provider', array('user_domain' => $user_domain));
                return false;
            }
        }
        return true;
