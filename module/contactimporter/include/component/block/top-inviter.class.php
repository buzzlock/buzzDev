...
class Contactimporter_Component_Block_Top_inviter extends Phpfox_Component
{
	...
	public function process()
	{
		...
		-> select('pu.user_name,pi.user_id,pu.full_name,pu.email as inviter_email,pi.emails as emails,pi.socials as socials,sum(emails + socials) as number_invitation') 
		-> from(phpfox::getT('contactimporter_statistics'), 'pi') 
		-> leftJoin(phpfox::getT('user'), 'pu', 'pu.user_id = pi.user_id')
		-> group('pi.user_id') -> where(isset($aCond)) 
		-> order('number_invitation DESC') -> limit($sLimit) -> execute('getRows');
