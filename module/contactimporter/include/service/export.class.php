...
class Contactimporter_Service_Export extends Phpfox_Service
{
	public function exportCSV($userID)
	{
		...
		$csv_separator = ",";
		$error = '';
		$aRows = $this -> database() -> select("u.*") -> from(phpfox::getT('user'), 'u') -> leftjoin(Phpfox::getT('friend'), 'fr', 'u.user_id = fr.user_id') -> where('fr.friend_user_id = ' . $userID) -> execute('getSlaveRows');
