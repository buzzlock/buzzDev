...
<div id="provider-list">
	<table id="title">
		<tr>
			...
			<td class="providers-active">{phrase var='contactimporter.enabled'}</td>
		</tr>
	</table>
	<table id="table-2">
		...
		<tr id="{$provider.name}" class="checkRow{if $provider.enable eq 0} tr{else}{/if}" style="cursor:move;">
			...
			<div id="update_active_{$provider.name}">
				<a style="cursor:pointer;text-decoration: underline;" href="javascript:updateprovideractive('{$provider.name}',{$provider.enable});">{if $provider.enable eq 1}{phrase var='core.yes'}{else}{phrase var='core.no'}{/if}</a>
			</div></td>
		</tr>
		{/foreach}
	</table>
