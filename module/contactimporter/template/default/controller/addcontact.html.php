...
{ if isset($aErrors) and  count($aErrors)== 0}
	 <h1>{phrase var='contactimporter.invite_friends'}</h1>
            <div>{phrase var='contactimporter.send_invitation_to_your_friends_successfully_1' contactimporter_link=$contactimporter_link homepage=$homepage}</div>
{else}
	<div>{phrase var='contactimporter.there_were_errors_when_you_send_the_invitation'}</div>
	{foreach from=$aErrors item=er}
