...
<div id="openinviter">
    ...
    {*<p class="description" style="margin-bottom:5px;">{phrase var='contactimporter.you_can_send_max_invitations_per_time' max=$yn_max_invitation}</p>*}
    <br />
    ...
    <div class="wrapper-list">
        <table class='thTableOddRow yncontact_email_header' align='left' cellspacing='0' cellpadding='5px' style="width: 100%; border:0px solid">
            <tr>
                ...
                <td >{phrase var='contactimporter.name'}</td>
                <td style="width:150px">{phrase var='user.email'}</td>
            </tr>
        </table>
        <div id="div_list_view" style="width:100%;padding-left:0px;">
            <table class='thTableOddRow' cellspacing='0' cellpadding='5px' style="width:100%; border:0px solid">
                {php}$counter=0;{/php}
                ...
                <tr class='thTableOddRow yncontact_email_contact' id="row_{php}echo ++$counter;{/php}">
                    <td align="left">
                        <input id="check_{php}echo $counter;{/php}" class="contact_item" type="checkbox" name="items[]" value="{$aInvite.id}" />
                    </td>
                    <td onclick='check_toggle({php}echo $counter;{/php},document.getElementById("row_{php}echo $counter;{/php}"),true);'>{$aInvite.name}</td>
                    <td align="right">
                        {if $aInvite.pic eq ''}
    ...
    <form method="post" action="{url link='contactimporter.invite'}" class="global_form yncontact_emal_invite" name='openinviterform' id="openinviterform" enctype="application/x-www-form-urlencoded" onsubmit="return check_select_invite();">
        ...
        <div class='extra_info' style='margin-top:20px'>
        	<ul>
            	<li>{phrase var='contactimporter.notice_quota_sending'} </li>
            	<li>{phrase var='contactimporter.maximum_invitation_per_day' number=$iQuota} </li>
            	<li>{phrase var='contactimporter.maximum_invitation_per_day_of_provider_number' provider=$sProviderName number=$iProviderQuota}</li>
            </ul>
        </div>
        <div class="yncontact_custom_message">
            ...
            {/if}
            <input id="skip-button" class="button" type='button' value ="{phrase var='contactimporter.skip'} &gt;&gt;" onclick="document.getElementById('skip').submit();">
        </div>
