...
<div class="extra_info">
	{phrase var='fevent.no_upcoming_events'}
	<ul class="action">
		<li><a href="{url link='fevent.add' module=$aEventParent.module item=$aEventParent.item}">{phrase var='fevent.add_an_event'}</a></li>
	</ul>
</div>
{else}
...
<div class="{if is_int($phpfox.iteration.events/2)}row1{else}row2{/if}{if $phpfox.iteration.events == 1} row_first{/if}">
	...
	<div style="margin-left:60px; min-height:55px; height:auto !important; height:55px;">	
		...
		{/if}	
		<div class="extra_info">
			{phrase var='fevent.time_stamp_at_location' time_stamp=$aEvent.start_time_stamp location=$aEvent.location_clean}{if !empty($aEvent.city)}, {$aEvent.city|clean}{/if}, {$aEvent.country_iso|location}
		</div>		
	</div>
</div>
{/foreach}
