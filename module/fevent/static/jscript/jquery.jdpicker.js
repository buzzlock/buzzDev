		...
		if (!this.currentMonth || !(this.currentMonth.getFullYear() == newMonth.getFullYear() &&
		  ...
		  for (var i = 0; i <= numDays; i++) {
			...
			if ((this.select_week == 0 && currentDay.getMonth() == date.getMonth() && this.isNewDateAllowed(currentDay) && !this.isHoliday(currentDay)) || (this.select_week==1 && currentDay.getMonth() == date.getMonth() && this.isNewDateAllowed(firstDayOfWeek))) {
			  ...
			  if(typeof(POOL.search[this.dateToString(currentDay)])!='undefined'){
			  	isAvailable = true;
			  	totalEvents = POOL.search[this.dateToString(currentDay)].length;
			  	tooltip = 'onmouseover="$(\'#tooltip\').html(\''+POOL.phrase_events+': <b>'+totalEvents+'</b>\'); $(\'#tooltip\').show();" onmouseout="$(\'#tooltip\').hide();" onmousemove="var _x = event.pageX ? event.pageX : event.clientX; var _y = event.pageY ? event.pageY : event.clientY + $(document).scrollTop(); $(\'#tooltip\').css(\'left\', _x+10); $(\'#tooltip\').css(\'top\', _y+10);"'; 
			  }
			  dayCells += '<td '+tooltip+' class="selectable_day'+(isAvailable ? ' available' : '')+'" date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
			} else {
		  ...
		  if(this.select_week == 0){
			  $(".selectable_day", this.tbody).click(this.bindToObj(function(event) {
                ...
                url = url.replace(/\/when_.*?\//g, '/');
                url = url.replace(/\/date_.*?\//g, '/');
                url = url.replace(/\/location_.*?\//g, '/');
                //url = url.replace(/\/view_.*?\//g, '/');
                url = url.replace(/\/search-id_.*?\//g, '/');
                url += 'date_'+$(event.target).attr("date").replace(/\//g,'-')+'/';
                url += 'when_all-time/';
                window.location = url;
			  }));
		  }else{
