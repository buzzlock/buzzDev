...
class AdvancedMarketplace_Component_Controller_Admincp_Index extends Phpfox_Component
{
	/**
	...
	public function process()
	{
		...
		$this->template()->setTitle(Phpfox::getPhrase('advancedmarketplace.manage_categories'))
			->setBreadcrumb(Phpfox::getPhrase('advancedmarketplace.manage_categories'), $this->url()->makeUrl('admincp.advancedmarketplace'))
			->setPhrase(array(
			...
			->setHeader(array(
					...
					'<script type="text/javascript">$Behavior.initIndex = function() { $Core.advancedmarketplace.url(\'' . $this->url()->makeUrl('admincp.advancedmarketplace') . '\'); }</script>'
				)
			)
			->assign(array(
