...
class advancedmarketplace_Component_Controller_Search extends Phpfox_Component {
	...
	public function process()
	{
		...
		if ($this->request()->get('req3') == 'category')
		{
			...
			if ($aListingCategory = Phpfox::getService('advancedmarketplace.category')->getForEdit($sCategoryUrl))
			{
				/*$this->template()->setBreadCrumb(Phpfox::getPhrase('feedback.category'));
				...
				$this->template()->setBreadCrumb(Phpfox::getLib('locale')->convert($aFeedBackCategory['name']), $this->url()->makeUrl('current'), true);*/
				$this->search()->setFormUrl($this->url()->permalink(array('advancedmarketplace.search.category', 'view' => $this->request()->get('view')), $aListingCategory['category_id'], $aListingCategory['name']));
			}
		...
		elseif ($this->request()->get(($bIsProfile === true ? 'req4' : 'req3')) == 'tag')
		{
			...
			{
				$this->search()->setFormUrl($this->url()->permalink(array('advancedmarketplace.search.tag', 'view' => $this->request()->get('view')), $aTag['tag_text']));
		...
		if ($sCategoryUrl !== null)
		{
			...
			$this->template()->setBreadcrumb(Phpfox::getLib("locale")->convert($aCatesPathInvert[0]["name"]), false);
		}
	...
	private function _sortRecentTimeListing(&$aListings)
	{
		$iMax = 0;
		$iPos = 0;
		$aTemp = array();
		$iLength = count($aListings);
		...
		for($i = 0; $i < $iLength; $i++)
		{
			...
			{
				if($aListings[$iMax]['review_time'] < $aListings[$j]['review_time'])
				{
					$iMax = $j;
				}
			}
			if ($iMax != $i ) 
			{
				$this->_swapListing($aListings[$iMax], $aListings[$i]);
			}
		}
	}
	
	private function _swapListing(&$a, &$b)
	{
		$aTemp = $a;
		$a = $b;
		$b = $aTemp;
