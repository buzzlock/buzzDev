...
class AdvancedMarketplace_Service_Category_Process extends Phpfox_Service 
{
	/**
	 * Class constructor
	 */	
	...
	public function add($aVals)
	{
		if (empty($aVals['name']))
		{
			return Phpfox_Error::set(Phpfox::getPhrase('advancedmarketplace.provide_a_category_name'));
		}
		...
		if ($iCheck)
		{
			return Phpfox_Error::set(Phpfox::getPhrase('advancedmarketplace.this_category_name_cannot_be_used_due_to_that_there_is_already_another_category_with_the_same_name_being_used'));
		}		
		...
		$this->cache()->remove('advancedmarketplace', 'substr');
	...
	public function delete($iId)
	{
		...
		$aListings = $this->database()->select('m.listing_id, m.user_id, m.image_path')
			->from(Phpfox::getT('advancedmarketplace_category_data'), 'mcd')
			->join(Phpfox::getT('advancedmarketplace'), 'm', 'm.listing_id = mcd.listing_id')
			->where('mcd.category_id = ' . (int) $iId)
			->execute('getRows');		
			
		foreach ($aListings as $aListing)
		{
			Phpfox::getService('advancedmarketplace.process')->delete($aListing['listing_id'], $aListing);
		}
