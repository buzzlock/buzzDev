...
class AdvancedMarketplace_Service_Custom_Group extends Phpfox_Service
{
			...
			foreach($aGroupIds as $iId)
			{
				...
				foreach($aCustomFields as $aField)
				{
					if(!empty($aField))
					{
						...
						->execute('getRows');
						$aFields[$iId['group_id']][$aField['field_id']]['html'] = $this->_getConvertHtmlField($aField['var_type'], $aOptions, $aField['field_name']);
	...
	private function _getConvertHtmlField($var_type, $aOptions, $fieldname)
	{
		...
		$sHtml = '<div class="table_left">'.phpfox::getPhrase($fieldname).'</div>';
		...
		switch($var_type)
		{
			case 'select':
				...
				foreach($aOptions as $aOption)
				{
					$sHtml .= "<option value=\"".$aOption['option_id']."\">".phpfox::getPhrase($aOption['option_name'])."</option>";
				}
				$sHtml .= '</select><br/>';
				break;
			case 'textarea':
				$sHtml .= '<div class="table_right">';
				$sHtml .= '<textarea class="custom_textarea" cols="60" style="width:90%;" rows="8" name="custom">';
				$sHtml .= '</textarea><br/>';
				break;
			case 'text':
				$sHtml .= '<div class="table_right">';
			...
			case 'radio':
				...
				foreach ($aOptions as $aOption)
				{
					...
					{
						$sHtml .= '<input checked="checked" name="custom"  type="radio" value="'. $aOption['option_id'] .'"><label>' . Phpfox::getPhrase($aOption['option_name']) . '</label>';
					}
					else 
					{
						$sHtml .= '<input  type="radio" name="custom" value="'. $aOption['option_id'] .'"><label>' . Phpfox::getPhrase($aOption['option_name']) . '</label>';
					}
					$sHtml .= '</div>';
				}
				$sHtml .= '</div>';
			...
			case 'multiselect':
				...
				foreach ($aOptions as $aOption)
				{
					...
					{
						$sHtml .= '<option selected="selected"   value="'. $aOption['option_id'] .'">' . Phpfox::getPhrase($aOption['option_name']) . '</option>';
					}
					else 
					{
						$sHtml .= '<option value="'. $aOption['option_id'] .'">' . Phpfox::getPhrase($aOption['option_name']) . '</option>';
			...
			case 'checkbox':
				...
				foreach ($aOptions as $aOption)
				{
					...
					if(isset($aOption['selected']) && $aOption['selected'] == true)
					{
						$sHtml .= '<input checked="checked" name="custom"  type="checkbox" value="'. $aOption['option_id'] .'"><label>' . Phpfox::getPhrase($aOption['option_name']) . '</label>';
					}
					else 
					{
						$sHtml .= '<input  type="checkbox" name="custom" value="'. $aOption['option_id'] .'"><label>' . Phpfox::getPhrase($aOption['option_name']) . '</label>';
					}
					$sHtml .= '</div>';
				}
				$sHtml .= '</div>';
	...
	public function deleteGroup($iId)
	{
		...
		$this->database()->update(Phpfox::getT('advancedmarketplace_custom_field'), array('group_id' => 0), 'group_id = ' . $aGroup['group_id']);

		$this->database()->delete(phpfox::getT('advancedmarketplace_custom_group'), 'group_id = ' . $aGroup['group_id']);
