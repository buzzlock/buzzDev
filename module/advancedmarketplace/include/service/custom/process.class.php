...
class AdvancedMarketplace_Service_Custom_Process extends Phpfox_Service
{
	...
	public function addCustomGroup($aVals)
	{
		...
		if($iId)
		{
			...
			foreach ($aVals['category'] as $iCategory)
			{
				...
				phpfox::getLib('database')->insert(Phpfox::getT('advancedmarketplace_category_customgroup_data'), array('category_id'=>$iCategory, 'group_id' => $iId));
			}
		}
	...
	private function _addOptions($iFieldId, &$aVals)
	{
		...
		foreach ($aVals['option'] as $iKey => $aOptions)
		{
			...
			foreach ($aOptions as $sLang => $aOption)
			{
				...
				{
					$this->_aOptions[$iKey . $sLang] = $this->database()->insert(Phpfox::getT('advancedmarketplace_custom_option'), array(
						'field_id' => $iFieldId,
	...
	public function toggleFieldActivity($iId)
	{
		...
		$this->database()->update(Phpfox::getT('advancedmarketplace_custom_field'), array('is_active' => ($aField['is_active'] ? 0 : 1)), 'field_id = ' . $aField['field_id']);
	...
	public function toggleGroupActivity($iId)
	{
		...
		$this->database()->update(Phpfox::getT('advancedmarketplace_custom_group'), array('is_active' => ($aField['is_active'] ? 0 : 1)), 'group_id = ' . $aField['group_id']);
	...
	public function updateGroupOrder($aVals)
	{
		...
		{
			$this->database()->update(Phpfox::getT('advancedmarketplace_custom_group'), array('ordering' => (int) $iOrder), 'group_id = ' . (int) $iId);
		}
	...
	public function updateFieldOrder($aVals)
	{
		...
		foreach ($aVals as $iId => $iOrder)
		{
			$this->database()->update(Phpfox::getT('advancedmarketplace_custom_field'), array('ordering' => (int) $iOrder), 'field_id = ' . (int) $iId);
		}
	...
	public function deleteField($iId)
	{
		...
		$this->database()->delete(phpfox::getT('advancedmarketplace_custom_field'), 'field_id = ' . $aField['field_id']);
		$this->cache()->remove();
