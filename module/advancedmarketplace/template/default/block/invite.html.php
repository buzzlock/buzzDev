...
<div class="viewed_listing">
	{foreach from=$aEventInvites key=iKey item=aListing}
        ...
        <div class="row_listing_image">
			<a href="{$aListing.url}" title="{$aListing.title|parse|clean}">
                {if $aListing.image_path != NULL}
                    {img title=$aListing.title server_id=$aListing.server_id path='core.url_pic' file='advancedmarketplace/'.$aListing.image_path suffix='_120' max_width=60 max_height=60 }
                {else}
                    <img title="{$aListing.title}" src="{$corepath}module/advancedmarketplace/static/image/default/noimage.png" max_width='60' max_height='60' />
                {/if}
            </a>
        </div>
    ...
    <div class="clear"></div>
    {/foreach}
    {*{if $bIsViewMore}<div class="view_more_link"><a href="{url link='advancedmarketplace.view_invites'}">{phrase var='advancedmarketplace.view_more'}</a></div>{/if}*}
</div>
{/if}