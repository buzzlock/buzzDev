	...
	action: function(oObj, sAction)
	{
		...
		switch (sAction)
		{
			case 'edit':
				...
				break;
			case 'delete':
				if (confirm(oTranslations['advancedmarketplace.are_you_sure_this_will_delete_all_listings_that_belong_to_this_category_and_cannot_be_undone']))
				{
					window.location.href = this.sUrl + 'delete_' + aParams['id'] + '/';
				}
				break;
			case 'manage_customfield':
				tb_show(oTranslations['advancedmarketplace.admin_menu_manage_custom_fields'], $.ajaxBox('advancedmarketplace.showmanagecustomfieldpopup', 'height=230&width=800&lid=' + aParams['id']));
				break;
			default:
...
var _bh = {
	...
	pop : function() {
		var o = _bh.ii.pop();
		if (o != null)
			updateFeature(o.id, o.is_active);
	}
};
...
function updateFeature(listing_id, is_active)
{
	if(is_active == 0)
	{
...
function setGroupInterfaceActions(obj) {
	...
	obj.find(".yn_jh_cusgroup_delete").click(function(evt){
		evt.preventDefault();
		if (!confirm(oTranslations['core.are_you_sure'])) return false;
		var $this = $(this);
		obj.find(".ajxloader").show();
...
function setCustomFieldInterfaceActions(obj){
	...
	obj.find(".btndelete").click(function(evt){
		...
		$.ajaxCall("advancedmarketplace.removeCustomField", "cusfieldalias=" + $this.attr("ref"));
