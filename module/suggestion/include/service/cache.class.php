 ...
 */
require_once (PHPFOX_DIR . 'module' . PHPFOX_DS . 'suggestion' . PHPFOX_DS . 'include' . PHPFOX_DS . 'service' . PHPFOX_DS . 'libs.class.php');

class Suggestion_Service_Cache extends Younet_Service 
{
	/**
	 * Class constructor
	 */	
	...
	{	
		$this->_oCache = $this->cache();				
                ...
                $this->_sPrefix = 'suggestion'.PHPFOX_DS;
                
                //key list to handle all key, to know what to clean
                $this->_sKeyList = 'suggestion.key.list.all';
        ...
        public function get($sKey){
            ...
            if (isset($aResult['expired'])){//cached expired is set
        ...
        public function set($sKey, $oValue, $iExpired=''){
            $sKey = $this->buildKey($sKey);
//            $sValue = serialize($oValue);
            $aValue['data'] = $oValue;           
            
            if ($iExpired !== ''){ //has set expired time
