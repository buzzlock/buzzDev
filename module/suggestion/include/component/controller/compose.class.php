...
//                $sLink = base64_decode($this->request()->get('link'));
                $sTitle = $this->request()->get('link');
//                $sLink = Phpfox::getService('suggestion.url')->makeLink('#', $sTitle, false);
                $sLink = $sTitle;
                
                if ((int)$iUserId>0){
                    $sUserLink = Phpfox::getService('suggestion')->getUserLink($iUserId);
                    $sPhrases = Phpfox::getPhrase('suggestion.send_a_personal_message_to_user');
                    $sPhrases = preg_replace('/{{user_name}}/', $sUserLink, $sPhrases);
                    
                    $sHeader = Phpfox::getPhrase('suggestion.you_might_need_user_name_permission_to_join_data');                    
		...
		if (($iUserId = $this->request()->get('id')) || ($iUserId = $this->getParam('id')))
		{
			...
			if (isset($aUser['user_id']))
			{
				if (Phpfox::getService('mail')->canMessageUser($aUser['user_id']) == false)
				{
					...
					return Phpfox_Error::display(preg_replace('/{{user_name}}/', $sUserLink, Phpfox::getPhrase('suggestion.unable_to_send_a_private_message_to_this_user_at_the_moment')));
				}
