    ...
    {foreach from=$aRows item=aRow}          
        <p style="position: absolute">
        	{if $aRow.image_path==""}
        		{$aRow.img}
        	{else}
        		<a class="large_item_image" href="{permalink module='pages' id=$aRow.page_id title=$aRow.title}" title="{$aRow.title|clean}">
        		{img server_id=$aRow.image_server_id return_url=false path='core.url_pic' file='pages/'.$aRow.image_path suffix='_50' max_width=50}
        		</a>
        	{/if}
        </p>
        <p style="padding: 0 0 5px 60px; border-bottom:1px solid #f2f2f2;">
            <span class="l13">
                <a target="_blank" rel="{$aRow.is_right}" id="{$aRow.link}" href="{$aRow.link}" class="suggestion-join-pages" style="font-weight: bold;">{$aRow.title}
            ...
            <span class="l13">
                {if $aRow.isAllowSuggestion}
                    <a target="_blank" id="{$aRow.page_id}" class="suggest-page" href="#" rel="">{phrase var='suggestion.suggest_to_friends_2'}</a> 
                {/if}
