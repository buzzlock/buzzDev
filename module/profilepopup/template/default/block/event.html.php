        ...
        <div class="yn_profilepopup_hovercard_stage" {if !isset($aCoverPhoto)} style="padding-top: 10px;" {/if}>
                ...
                <div class="yn_profilepopup_hovercard_content">
                        ...
                        <div class="yn_profilepopup_image">
                                <a href="{permalink module='event' id=$aEvent.event_id title=$aEvent.title}">{img server_id=$aEvent.server_id title=$aEvent.title path='event.url_image' file=$aEvent.image_path suffix='_120' max_width='100' max_height='100'}</a>
                        </div>
                        {/if}
                        <div class="yn_profilepopup_main" {if !Phpfox::getParam('profilepopup.enable_thumbnails')}style="margin-left: 0px;"{/if}>
                                <div class="yn_profilepopup_main_title"> <a href="{permalink module='event' id=$aEvent.event_id title=$aEvent.title}" class="link">{$aEvent.title|clean|split:25}</a></div>
                                <div>
                                        <div class="yn_profilepopup_info yn_profilepopup_info_left">
                ...
                <div class="yn_profilepopup_hovercard_content">
                        ...
                        <div class="yn_profilepopup_main" {if !Phpfox::getParam('profilepopup.enable_thumbnails')}style="margin-left: 0px;"{/if}>
                                ...
                                <div>
                                        ...
                                        <div class="yn_profilepopup_info">
                                                <div class="yn_profilepopup_info_left">{$aItem.lang_name}:&nbsp;</div>
                                                <div class="yn_profilepopup_info_right">{$aEvent.event_date}&nbsp;</div>
                                        </div>
                                        {/if}
                                        <!-- Location -->
        ...
        <div class="yn_profilepopup_hovercard_footer">
                ...
                <ul class="yn_profilepopup_list_horizontal">
                        ...
                        {if $aEvent.view_id == 0 && ($aEvent.user_id == Phpfox::getUserId() && Phpfox::getUserParam('event.can_edit_own_event')) || Phpfox::getUserParam('event.can_edit_other_event')}
                                <li class="yn_profilepopup_list_item" >
                                        <a title="{phrase var='profilepopup.event_invite'}" class="buttonlink yn_profilepopup_icon_invite" href="{url link='event.add.invite' id=$aEvent.event_id}">{phrase var='profilepopup.event_invite'}</a>			
                                </li>                        
                        {/if}		
                        
                        {if !isset($aEvent.rsvp_id) || $aEvent.rsvp_id != 1}
                                <li class="yn_profilepopup_list_item" >
                                        <a title="{phrase var='profilepopup.event_join'}" onclick="ynfbpp.closePopup(); $.ajaxCall('profilepopup.joinEvent', 'id={$aEvent.event_id}&amp;rsvp=1'); return false;" class="buttonlink yn_profilepopup_icon_join" href="#">{phrase var='profilepopup.event_join'}</a>			
                                </li>                        
                        {else}
                                <li class="yn_profilepopup_list_item" >
                                        <a title="{phrase var='profilepopup.event_leave'}" onclick="ynfbpp.closePopup(); ynfbpp.leaveEvent({$aEvent.event_id}, '{phrase var='profilepopup.event_confirm_leave'}'); return false;" class="buttonlink yn_profilepopup_icon_leave" href="#">{phrase var='profilepopup.event_leave'}</a>			
                                </li>                        
                        {/if}
                </ul>
                {/if}
