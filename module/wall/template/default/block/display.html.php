...
{if isset($bForceFormOnly) && $bForceFormOnly}
	{template file='wall.block.form'}
{else}
	{if !isset($bIsViewMore) or !$bIsViewMore}
	{template file="wall.block.filter"}
	{/if}
...
<div {if !$bIsHashTagPop}id="js_feed_content"{/if} class="js_feed_content">
		...
		{else}
			<div>
				...
				<div class="timeline_left">			
					{if (!PHPFOX_IS_AJAX && Phpfox::getService('wall.feed')->timeline()) || Phpfox::getLib('request')->get('resettimeline')}
					...
					<div class="timeline_feed_row">
						...
						{template file='wall.block.timeline'}
					</div>		
					{/foreach}
				</div>
		...
		{else}
			{if count($aFeeds) || (isset($bForceReloadOnPage) && $bForceReloadOnPage)}
				<div id="feed_view_more">
					...
					<a {if !PHPFOX_IS_AJAX && isset($bForceReloadOnPage) && $bForceReloadOnPage} style="text-indent:-1000px; overflow:hidden; background:transparent; border:0px;"{/if} href="{if Phpfox::getLib('module')->getFullControllerName() == 'core.index-visitor'}{url link='core.index-visitor' page=$iFeedNextPage}{else}{url link='current' page=$iFeedNextPage}{/if}" onclick="$(this).hide(); $('#feed_view_more_loader').show(); $.ajaxCall('wall.viewMore', 'page={$iFeedNextPage}{if defined('PHPFOX_IS_USER_PROFILE') && isset($aUser.user_id)}&profile_user_id={$aUser.user_id}{/if}{if isset($aFeedCallback.module)}&callback_module_id={$aFeedCallback.module}&callback_item_id={$aFeedCallback.item_id}{/if}&year={$sTimelineYear}&month={$sTimelineMonth}'+ '&viewId=' + wallFillterViewId, 'GET'); return false;" class="global_view_more no_ajax_link">{phrase var='feed.view_more'}</a>
					{/if}
				</div>
			{else}
				{if Phpfox::getService('wall.feed')->timeline()}
	...
	{if Phpfox::getParam('feed.refresh_activity_feed') > 0 && Phpfox::getLib('module')->getFullControllerName() == 'core.index-member'}
		{if Phpfox::VERSION >= '3.6.0'}
			<script type="text/javascript">
				$Behavior.reloadActivityFeedAdvWall = function(){l}
	...
	{if !PHPFOX_IS_AJAX}
		{if Phpfox::VERSION >= '3.6.0'}
			<script type="text/javascript">
			 $Behavior.loadDisplayBlockAdvWall = function(){l}
				{if Phpfox::isUser() }
				Typeahead.data = {$aJSONFriends};
				Typeahead.emotions = {$aEmoticons};
				{/if}
				iProfileUserId  = '{if defined('PHPFOX_IS_USER_PROFILE') && isset($aUser.user_id)}{$aUser.user_id}{/if}';
	...
	{if !Phpfox::getService('wall.feed')->timeline() && !PHPFOX_IS_AJAX  && defined('PHPFOX_IS_USER_PROFILE')}
	<script type="text/javascript">
	//$Behavior.doFilterAdvWall = function(){l}
		//callDoFilter('all');
	//{r}	
	</script>
	{/if}
