...
<div class="activity_feed_form_share">
	...
	{if !isset($bSkipShare)}
		<ul class="activity_feed_form_attach">
			...
			{if $phpfox.iteration.feedlinks == 3 && Phpfox::getService('wall.feed')->timeline()}
			...
			{else}
				{if ($aFeedStatusLink.no_profile && !isset($bFeedIsParentItem) && (!defined('PHPFOX_IS_USER_PROFILE') || (defined('PHPFOX_IS_USER_PROFILE') && isset($aUser.user_id) && $aUser.user_id == Phpfox::getUserId()))) || !$aFeedStatusLink.no_profile}
					<li>
						<a href="#" style="background-image:url('{img theme='feed/'$aFeedStatusLink.icon'' return_url=true}'); background-repeat:no-repeat; background-position:{if Phpfox::getService('wall.feed')->timeline() && $phpfox.iteration.feedlinks >= 3}5px center{else}center left{/if};" rel="global_attachment_{$aFeedStatusLink.module_id}"{if $aFeedStatusLink.no_input} class="no_text_input"{/if}>
							<div>
			...
			{if $phpfox.iteration.feedlinks == count($aFeedStatusLinks) && Phpfox::getService('wall.feed')->timeline()}
				</ul>
			</li>			
			{/if}			
	...
	<form method="post" action="#" id="js_activity_feed_form" enctype="multipart/form-data">
		...
		<div class="activity_feed_form_holder">		
			...
			<div class="global_attachment_holder_section" id="global_attachment_status" style="display:block;">
				<div id="js_status_attachment_link_cancel" class="link_attachment_cancel_link p_4 t_right"><a href="#" onclick="$Core.cancelParseLinkInFeed(); return false;">{phrase var='link.cancel'}</a></div>
				...
				<div class="_uiPostComposer">
					<div class="_uiMentionTypeAheadContainer" id="user_status_area">
						...
						<div class="_uiTypeaheaMentiondArea">
							<div class="innerWrap">
								<textarea placeholder="{if isset($aFeedCallback.module) || defined('PHPFOX_IS_USER_PROFILE')}{phrase var='feed.write_something'}{else}{phrase var='feed.what_s_on_your_mind'}{/if}" {if isset($aPage)} id="pageFeedTextarea" {else} {if isset($aEvent)} id="eventFeedTextarea" {else} {if isset($bOwnProfile) && $bOwnProfile == false}id="profileFeedTextarea" {/if}{/if}{/if} cols="60" rows="8" name="val[user_status_plain_text]" class="textarea_post_composer textarea_post_user_status _uiMentionTypeahead textareaNoResize">{if isset($aFeedCallback.module) || defined('PHPFOX_IS_USER_PROFILE')}{phrase var='feed.write_something'}{else}{phrase var='feed.what_s_on_your_mind'}{/if}</textarea>
							</div>
						</div>
		...
		<div class="activity_feed_form_button">
			...
			<div class="activity_feed_form_button_position">
				...
				{if ((defined('PHPFOX_IS_PAGES_VIEW') && $aPage.is_admin) || ((Phpfox::isModule('share') && !defined('PHPFOX_IS_USER_PROFILE') && !defined('PHPFOX_IS_PAGES_VIEW') && !defined('PHPFOX_IS_EVENT_VIEW') && ((Phpfox::getParam('share.share_on_facebook') && Phpfox::getParam('facebook.facebook_app_id') && Phpfox::getParam('facebook.facebook_secret')) || Phpfox::getParam('share.share_on_twitter'))) || (defined('PHPFOX_IS_USER_PROFILE') && isset($aUser.user_id) && $aUser.user_id == Phpfox::getUserId() && Phpfox::getService('wall.feed')->timeline() && Phpfox::getParam('feed.can_add_past_dates'))))}
					...
					<div id="activity_feed_share_this_one">
						<ul>
							...
							{/if}
							{if ((defined('PHPFOX_IS_PAGES_VIEW') && $aPage.is_admin && Phpfox::getService('wall.feed')->timeline()) || (defined('PHPFOX_IS_USER_PROFILE') && isset($aUser.user_id) && $aUser.user_id == Phpfox::getUserId() && Phpfox::getService('wall.feed')->timeline() && Phpfox::getParam('feed.can_add_past_dates')))}
							<li>
							...
							{/if}						
							{if $bLoadCheckIn}
								{template file='wall.block.checkin'}						
							{/if}
						</ul>
						<div class="clear"></div>
					</div>
				...
				{/if}
				{if ((defined('PHPFOX_IS_PAGES_VIEW') && $aPage.is_admin) || (defined('PHPFOX_IS_USER_PROFILE') && isset($aUser.user_id) && $aUser.user_id == Phpfox::getUserId() && Phpfox::getService('wall.feed')->timeline() && Phpfox::getParam('feed.can_add_past_dates')))}
