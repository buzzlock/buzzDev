 ...
 */
class Wall_Service_Feed extends Phpfox_Service 
{	
	...
	function getFilterByTypeCondition($sViewId){
		...
		if(!$havingSocialStream or !Phpfox::isModule('socialbridge'))
		{
			$aConds[] = "(feed.type_id NOT IN ('socialstream_facebook','socialstream_twitter'))";
		}
		else
		{
			$aProviders = Phpfox::getService('socialbridge')->getAllProviderData(Phpfox::getUserId());
						
			if(!$aProviders['facebook']['connected'])
			{
                $aConds[] = "(feed.type_id != 'socialstream_facebook')";
			}
            if(!$aProviders['twitter']['connected']){
            	
				$aConds[] = "(feed.type_id != 'socialstream_twitter')";                
            }
		}
	
		if($sViewId == 'network_only')
		{
			$aConds[] = "(feed.type_id != 'socialstream_facebook' AND feed.type_id != 'socialstream_twitter')";
		}
		
		if($sViewId  != 'all' && $sViewId != 'network_only')
		{
			$aConds[] = "(feed.type_id='{$sViewId}')";
		}
	...
	public function _hashSearch()
	{
		...
		} else 
		{
			...
			$this->database()->join(Phpfox::getT('tag'), 'hashtag', 'hashtag.item_id = feed.item_id AND hashtag.category_id = feed.type_id AND tag_text = \'' . Phpfox::getLib('database')->escape($sTag) . '\'');
		}
	...
	*	but it also controls if the entire div should be shown or not, including the dislikes
	...
	public function getPhraseForLikes(&$aFeed, $bForce = false)
	{
	    ...
	    if (isset($aFeed['likes']) && is_array($aFeed['likes']) && $iCountLikes > 0)
	    {
			...
			// else if (count($aLikes) > 0 || (isset($sTempUser) && !empty($sTempUser) && !empty($sTrimPhrase))) 
	...
	public function get($iUserid = null, $iFeedId = null, $iPage = 0, $bForceReturn = false,$sViewId = 'all')
	{

		$oUrl = Phpfox::getLib('url');
		$oReq = Phpfox::getLib('request');
		$oParseOutput = Phpfox::getLib('parse.output');
		$sFilterByTypeCondition  =  $this->getFilterByTypeCondition($sViewId);
		...
		{            
            $aRows = $this->database()->select('feed.*, apps.app_title, ' . Phpfox::getUserField().', u.view_id')
				...
				->where(isset($aCustomCondition) ? $aCustomCondition : array('feed.feed_id = ' . (int) $iFeedId . ' AND feed.user_id = ' . (int) $iUserid))
				->order('feed.time_stamp DESC')
				->limit(1)			
				->execute('getSlaveRows');			
		}
		...
		//if (Phpfox::getService('wall.feed')->timeline()|| Phpfox::getParam('feed.cache_each_feed_entry'))
		...
		{			
			...
			if (($aReturn = $this->_processFeed($aRow, $sKey, $iUserid, $bFirstCheckOnComments)))
			{
				...
				if(Phpfox::isModule('like'))
				{
					...
					//	Adv Wall: get feed_like_phrase, it is bug in default wall of phpFox 
					if(isset($aReturn['more_feed_rows']) && is_array($aReturn['more_feed_rows']))
					{
						foreach ($aReturn['more_feed_rows'] as $key => $value) {
							$this->getPhraseForLikes($aReturn['more_feed_rows'][$key]);
						}
					}					
				}				
		...
		if (Phpfox::isModule('ad') && Phpfox::getParam('ad.multi_ad') && $iFeedId == null && ( ($iAd = Phpfox::getService('ad')->getSponsoredFeed()) != false))
		{
	...
	public function processAjax($iId)
	{
		...
		if (isset($aFeeds[0]['type_id']))
		{
			Phpfox::getLib('template')->assign(array(
				...
				))->getTemplate((Phpfox::getService('wall.feed')->timeline() ? 'feed.block.timeline' : 'feed.block.entry'));				
		}
		else
		{
		...
		{	
			...
			*/
			...
			{	
				...
				// $aLikesList['likes'] = Phpfox::getService('like')->getLikesForFeed($aFeed['like_type_id'], (isset($aFeed['like_item_id']) ? $aFeed['like_item_id'] : $aRow['item_id']), ((int) $aFeed['feed_is_liked'] > 0 ? true : false), Phpfox::getParam('feed.total_likes_to_display'), true);
				// foreach ($aLikesList['likes'] as $aLike)
				// {
	...
	function timeline()
	{
		$obj  = Phpfox::getService('profile');
		
		if(method_exists($obj, 'timeline')){
			return $obj->timeline();
		}
