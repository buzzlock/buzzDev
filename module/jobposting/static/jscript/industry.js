...
$Behavior.addNewIndustry = function()
{
	...
	$('.popup_js_mp_category_list').change(function()
	{
		var iParentId = parseInt(this.id.replace('popup_js_mp_id_', ''));
		
		$('.popup_js_mp_category_list').each(function()
		{
			if (parseInt(this.id.replace('popup_js_mp_id_', '')) > iParentId)
			{
				$('#popup_js_mp_holder_' + this.id.replace('popup_js_mp_id_', '')).hide();				
				
				this.value = '';
			}
		});
		
		$('#popup_js_mp_holder_' + $(this).val()).show();
	});	
}