  ...
  {foreach from=$aCategories item=aCategory}
    <li class="{if $iCategoryFundraisingView == $aCategory.category_id} active{/if}"><a href="{$aCategory.url}" class="ajax_link">{$aCategory.name|convert|clean}</a>
                ...
                <ul>
                   ...
                   <li {if $iKey >= Phpfox::getParam('jobposting.subcategories_to_show_at_first')}style="display:none;" class="{if isset($sModule)}{$sModule}_{/if}subcategory_{$aCategory.category_id} special_subcategory"{/if}>
                      ...
                      {if $iKey >= Phpfox::getParam('jobposting.subcategories_to_show_at_first') && Phpfox::getParam('jobposting.subcategories_to_show_at_first') > 0}
