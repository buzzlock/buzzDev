...
class Jobposting_Component_Controller_Admincp_Category_Index extends Phpfox_Component
{
	...
	public function process()
	{
		...
		if ($iDelete = $this->request()->getInt('delete'))
		{
			...
			{
				$this->url()->send('admincp.jobposting.category', null, Phpfox::getPhrase('jobposting.category_successfully_deleted'));
			}
		}
		
		$this->template()->setTitle(Phpfox::getPhrase('jobposting.manage_industries'))
			->setBreadcrumb(Phpfox::getPhrase('jobposting.manage_industries'), $this->url()->makeUrl('admincp.jobposting.category'))
			->setPhrase(array('jobposting.are_you_sure_this_will_delete_all_items_that_belong_to_this_category_and_cannot_be_undone'))
			->setHeader(array(
					'jquery/ui.js' => 'static_script',
					'admin.js' => 'module_jobposting',
					'<script type="text/javascript">$Behavior.setURLJobposting = function() { $Core.jobposting.url(\'' . $this->url()->makeUrl('admincp.jobposting.category') . '\'); } </script>'
				)
			)
			->assign(array(
