...
class Jobposting_Service_Category_Category extends Phpfox_Service 
{
	private $_sOutput = '';
	...
	{		
		$sCacheId = $this->cache()->set('jobposting_category_browse' . ($iCategoryId === null ? '' : '_' . $iCategoryId));
	...
	public function getCategoriesById($iId = null, &$aCategories = null)
	{
		...
		{			
			...
			{				
				$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategory['name']), Phpfox::permalink('jobposting.category', $aCategory['category_id'], $aCategory['name']));
			}
		}		
		else 
		{			
			$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategories[0]['name']), Phpfox::permalink('jobposting.category', $aCategories[0]['category_id'], $aCategories[0]['name']));
		}
			...
			else 
			{
				...
				if($type==1 || $type==3){
					if($type==1)
						...
						$this->_sOutput .= '<select name="val[category][' . $iParentId . ']" class="popup_js_mp_category_list" id="popup_js_mp_id_' . $iParentId . '">' . "\n";
					}
					
				}
					
				else
					$this->_sOutput .= '<select name="search[category][' . "search_".$iParentId . ']" class="js_mp_category_list" id="js_mp_id_' . $iParentId . '">' . "\n";
				
				$this->_sOutput .= '<option value="">' . ($iParentId === 0 ? Phpfox::getPhrase('jobposting.select') : Phpfox::getPhrase('jobposting.select_a_sub_industry')) . ':</option>' . "\n";
			}
			...
			foreach ($aCategories as $iKey => $aCategory)
			{
				...
				if ($this->_sDisplay == 'option')
				{
					if(!PHpfox::isAdmin() || ($aCategory['parent_id']==0 && PHpfox::isAdmin()))
					{
						...
						else
						{
							...
							$this->_sOutput .= '<option value="' . $aCategory['category_id'] . '" id="popup_js_mp_category_item_' . $aCategory['category_id'] . '">' . ($this->_iCnt > 0 ? str_repeat('&nbsp;', ($this->_iCnt * 2)) . ' ' : '') . Phpfox::getLib('locale')->convert($aCategory['name']) . '</option>' . "\n";
						}	
				...
				else 
				{
					if($type!=3)
					{
						...
						$this->_sOutput .= '<option value="' . $aCategory['category_id'] . '" id="popup_js_mp_category_item_' . $aCategory['category_id'] . '">' . Phpfox::getLib('locale')->convert($aCategory['name']) . '</option>' . "\n";	
					}
				}
			}
   	...
   	private function _getForAdd($iNo, $iParentId, $iActive = null, $aCats = array())
	{
		...
		if (count($aCategories))
		{
			...
			$this->_sOutput .= '<select name="val[category][' . $iNo . '][' . $iParentId . ']" class="js_mp_category_list" id="js_mp_id_' . $iNo . '_' . $iParentId . '">' . "\n";
			$this->_sOutput .= '<option value="">' . ($iParentId === 0 ? Phpfox::getPhrase('jobposting.select') : Phpfox::getPhrase('jobposting.select_a_sub_industry')) . ':</option>' . "\n";
			...
			foreach ($aCategories as $iKey => $aCategory)
			{
				...
				//if($type!=3)
					$this->_sOutput .= '<option value="' . $aCategory['category_id'] . '" id="js_mp_category_item_' . $iNo . '_' . $aCategory['category_id'] . '"' . $sSelect . '>' . Phpfox::getLib('locale')->convert($aCategory['name']) . '</option>' . "\n";
				//else
				//	$this->_sOutput .= '<option value="' . $aCategory['category_id'] . '" id="popup_js_mp_category_item_' . $iNo . '_' . $aCategory['category_id'] . '"' . $sSelect . '>' . Phpfox::getLib('locale')->convert($aCategory['name']) . '</option>' . "\n";
			}
	...
	public function getPhraseCategory($company_id){
		$str = "";
