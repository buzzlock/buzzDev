...
class Jobposting_Service_Category_Process extends Phpfox_Service 
{
	...
	public function delete($iId)
	{
		...
		{
			Phpfox_Error::set(Phpfox::getPhrase('jobposting.you_can_only_delete_empty_industry'));
			return false;
		}
		...
		$aItems = $this->database()->select('m.campaign_id, m.user_id, m.image_path')
			->from(Phpfox::getT('jobposting_category_data'), 'mcd')
			->join(Phpfox::getT('jobposting'), 'm', 'm.campaign__id = mcd.campaign_id')
			->where('mcd.category_id = ' . (int) $iId)
			->execute('getRows');		
			
		foreach ($aItems as $aItem)
		{
			Phpfox::getService('jobposting.process')->delete($aItem['campaign_id'], $aItem);
		}
