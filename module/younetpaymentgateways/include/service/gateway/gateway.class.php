 ...
 */
class Younetpaymentgateways_Service_Gateway_Gateway extends Phpfox_Service 
{
	/**
	 * Class constructor
	 */	
	...
	{	
		$this->_sTable = Phpfox::getT('younetpaymentgateways_gatewayapi');	
	}
	
        public function getActiveGatewaySetting($sGatewayId)
        {
            $aRow = $this->database()->select('*')
                             ->from(Phpfox::getT('younetpaymentgateways_gatewayapi'))
	...
	public function getActive()
	{
		...
		{
			$oGateway = Phpfox::getService('younetpaymentgateways')->load($aGateway['gateway_id'], $aGateway);
	...
	public function get($aGatewayData = array())
	{
		...
		foreach ($aGateways as $iKey => $aGateway)
		{
			...
			if (!($oGateway = Phpfox::getService('younetpaymentgateways')->load($aGateway['gateway_id'], array_merge($aGateway, $aGatewayData))))
			{				
				unset($aGateways[$iKey]);
	...
	public function callback($sGateway)
	{
		...
		if (empty($sGateway))
		{
			...
			Phpfox::getService('younetpaymentgateways.gateway.process')->addLog(null, Phpfox::endLog());
		...
		if (!($oGateway = Phpfox::getService('younetpaymentgateways')->load($aGateway['gateway_id'], $_REQUEST)))
		{
			Phpfox::log('Unable to load gateway.');
			Phpfox::getService('younetpaymentgateways.gateway.process')->addLog($aGateway['gateway_id'], Phpfox::endLog());
	...
	public function __call($sMethod, $aArguments)
	{
		 ...
		 */
		if ($sPlugin = Phpfox_Plugin::get('younetpaymentgateways.service_gateway_gateway__call'))
		{
			return eval($sPlugin);
		}
