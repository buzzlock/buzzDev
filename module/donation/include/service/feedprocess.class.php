...
class Donation_Service_Feedprocess extends Phpfox_Service {
    ...
    public function add($sType, $iItemId, $iPrivacy = 0, $iPrivacyComment = 0, $iParentUserId = 0, $iOwnerUserId = null, $bIsTag = 0)
    {
        ...
        {
//			Phpfox::log('iscallback:' . serialize($aInsert));
        ...
        if ($this->_bIsCallback && $this->_aCallback['module'] == 'pages' && !$this->_bIsNewLoop && $iParentUserId > 0)
        {
            ...
            else
            {
                ...
                $this->_bIsCallback = false;
                $this->_aCallback = array();
                $this->add($sType, $iItemId, $iPrivacy, $iPrivacyComment, $iParentUserId, $aInsert['user_id']);
            }
        }
    ...
    public function addComment($aVals)
    {
        ...
        if (!isset($aVals['feed_reference']) || empty($aVals['feed_reference']))
        {
            Phpfox::getLib('mail')
                    ...
                    ->subject(Phpfox::getPhrase('feed.full_name_wrote_a_comment_on_your_wall', array('full_name' => Phpfox::getUserBy('full_name'))))
                    ->message(Phpfox::getPhrase('feed.full_name_wrote_a_comment_on_your_wall_message', array('full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink)))
